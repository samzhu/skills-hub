...
test.describe('S140 — E2E Critical Path Backfill', () => {
  test('AC-2: Skill 詳情頁顯示完整 SKILL.md + 版本 + Quality Score @S140 @ac-2 @happy-path @profile-single', async ({
    ...
    await test.step('Then 詳情頁渲染：name H1 + author + 版本 v1.0.0 + 風險 badge + 下載次數 + Quality 區塊', async () => {
      // Hero: H1 skill name + "by alice" + version pill
      ...
      // 4 MetricCard labels — 下載次數 / 版本 / 分類 / 建立時間 per SkillDetailPage line 148
      await expect(page.getByText('下載次數', { exact: true })).toBeVisible();
      await expect(page.getByText('分類', { exact: true })).toBeVisible();
      // 下載 metric value: "0 次下載"（seed skill 尚無 download events）
      await expect(page.getByText(/^\s*0\s*次下載\s*$/)).toBeVisible();

      // 概要 tab — 描述（對應 SKILL.md description frontmatter 同義 surface；
      // seed 用 description 合成 SKILL.md，故描述即為 markdown source 來源）
      await expect(page.getByText('Helper skill for orchestrating docker-compose dev stacks.')).toBeVisible();
      ...
      // 版本歷史 tab — click 後顯示 v1.0.0 version row
      await page.getByRole('tab', { name: '版本歷史' }).click();
