...
## Tick 107 — Mode B Round 60

Cut axis: **Control-behavior alignment**（chip / button label 與 behavior 1:1 mapping）

| # | 控制元件 | 所在位置 | Label | 實際 behavior | 對齊？ |
|---|---------|---------|-------|--------------|--------|
| 1 | Sort chips | HomePage | 推薦/最新/風險低/下載最多 | 推薦 → downloadCount,desc（intentional S106 MVP trade-off，code 已標注）；最新 → createdAt,desc；風險低 → riskLevel,asc；下載最多 → downloadCount,desc | ✅（trade-off 已文件化） |
| 2 | TabPill | MySkillsPage | 全部/已發布/草稿/已停用 | 全部=無 filter；各 tab → filter by status | ✅ |
| 3 | VoteButton | RequestBoardPage row | 投票 / 已投票，再點取消（aria-label） | toggleVote mutation；`aria-pressed` 同步；optimistic ±1 count | ✅ |
| 4 | RequestActionBar | RequestBoardPage row | 認領/釋放/上架完成/刪除/查看技能 | 各動作精確對應 API mutation；查看技能 → Link to skill | ✅ |
| 5 | RiskFilterSidebar | HomePage / CollectionsPage | 全部（重置）/ 無/低/中/高風險 | 全部 → onClear (clear Set)；tier → onToggle (add/remove)；active highlight 對應 selected state | ✅ |
| 6 | CategorySidebar | HomePage | 全部 / 各分類名稱 | 全部 → onSelect(null)；分類 → onSelect(name)；active 對應 selected | ✅ |
| 7 | Filter chips | NotificationsPage | 全部/回報/評論/需求 | setFilter → useNotifications(category\|undefined)；active chip = current filter | ✅ |
| 8 | Tabs (Radix) | SkillDetailPage | SKILL.md/品質/版本/評論/安全性/旗標/檔案 | 各 TabsContent 精確對應；activeTab state 同步 | ✅ |
| 9 | Clickable hero cards | HeroMetricsRow | QualityHeroCard / SecurityHeroCard | onClick → onTabChange('quality') / ('security')；active prop 控 highlight | ✅ |
| 10 | Action buttons | PageHeader | 分享/訂閱/已訂閱/下載技能 | 分享→ShareModal；訂閱 toggle→subscribe/unsubscribe mutate；下載→onDownload | ✅ |
| 11 | DropdownMenuItems | MarkdownActionMenu | 複製為 Markdown / 開啟 Markdown | copy()→clipboard；<a href> → opens raw skill.md | ✅ |
| 12 | Mode tabs | PublishPage | 上傳檔案 / 貼上文本 | setMode('file') / setMode('text')；UI 區塊即時切換 | ✅ |
| 13 | Modal buttons | 6 modal components | 取消/送出/儲存/新增/移除/關閉 | 各按鈕精確對應 onClose / mutate / handleSave / handleSubmit / handleRevoke | ✅ |
| 14 | StarButton | PageHeader | 訂閱/已訂閱 | aria 用 title；視覺文字精確反映 subscribed state；toggle 1:1 | ✅ |

**0 bugs found。**

所有 chip / button label 與 behavior 精確 1:1 對應。
唯一值得記錄的是 HomePage 的「推薦」chip 與「下載最多」共用 `downloadCount,desc` 排序，
這是已文件化的 S106 設計決策（「暫 = downloadCount,desc，future evolve 為 recommendation algorithm」），
不算 alignment bug — 日後擴充 recommendation 演算法時只需改 sortMap 中的一行。
