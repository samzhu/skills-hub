...
## 驗證方式
執行：`cd backend && ./gradlew test --tests '*VersionLabel*' --tests '*SkillCommand*'`

執行：`cd frontend && npm test -- --run`

執行：`rg -n "MAJOR\\.MINOR\\.PATCH|格式.*semver|必填.*版本" frontend/src docs/grimo/PRD.md docs/grimo/glossary.md docs/grimo/architecture.md`
