...
## Result
Date: 2026-05-17
Test: `AC-S187-1: 詳情頁編輯按鈕導向 edit page`; `AC-S187-2: 版本頁籤只顯示 Version History` (`frontend/src/pages/SkillDetailPage.test.tsx`)
Files changed:
- `frontend/src/App.tsx` (modified)
- `frontend/src/pages/SkillDetailPage.tsx` (modified)
- `frontend/src/pages/SkillDetailPage.test.tsx` (modified)
Notes: Red phase `cd frontend && npm test -- SkillDetailPage` failed on both S187-T01 assertions before implementation. Green phase passes with edit button navigating to `/skills/{id}/edit` and detail Versions tab rendering only `VersionsTabV2`; `cd frontend && npm run verify` also passes.
