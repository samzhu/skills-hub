...
## 實作結果
- `SkillCommandService.addVersion(...)` 已移除 S032 `SKILL.md name` 與平台 `skills.name` 必須相同的 guard。
- `SkillUploadAllowedToolsTest` 的舊 `AC-S032` 失敗情境改成 `AC-S176-5`：平台 name 與 package name 不同時，`addVersion` 成功，`latestVersion` 更新成 `1.1.0`，且 `skill_versions.frontmatter.name` 保存新 package name。
- 同一個測試同時確認重複版本檢查仍保留：第二次新增 `1.1.0` 仍丟 `VersionExistsException`。
- `SearchProjection` 內過時的 S032 註解已更新，避免後續維護者誤以為 package name 仍等於平台 name。

## 驗證結果
Red：

```bash
./gradlew test --tests "*SkillUploadAllowedToolsTest" --tests "*SkillUploadExplicitNameTest"
```

Result: `SkillUploadAllowedToolsTest > AC-S176-5... FAILED`，`commandService.addVersion(...)` 在舊 guard 丟 `IllegalArgumentException`。
...
Result: `BUILD SUCCESSFUL in 2m 31s`
