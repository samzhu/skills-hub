...
- [frontend/src/components/SkillCard.test.tsx](/Users/samzhu/workspace/github-samzhu/skills-hub/frontend/src/components/SkillCard.test.tsx) 把既有 score badge 測試明確標成 `AC-S193-5`，確認 `0.873` 顯示為 `87% 相符`。
- [frontend/src/pages/HomePage.test.tsx](/Users/samzhu/workspace/github-samzhu/skills-hub/frontend/src/pages/HomePage.test.tsx) 補 assembled `/browse` path 測試：semantic API response `score: 0.91` 會在 skill card 顯示 `91% 相符`，且不打 `/api/v1/skills?keyword=dd`。
- [e2e/tests/S193-semantic-search-score.spec.ts](/Users/samzhu/workspace/github-samzhu/skills-hub/e2e/tests/S193-semantic-search-score.spec.ts) 新增 Playwright browser evidence：`/browse` 輸入 `images and containers in CI` 後，semantic API 回傳第一筆 `score`，畫面顯示對應 `% 相符`，request log 只含 `/api/v1/search/semantic?q=`、不含 keyword API。
- Production UI / threshold / semantic formula 不變；此 task 只補 score visibility evidence。
...
- Local release gate：`./scripts/verify-all.sh` → PASS；V01=PASS、V02=INFO（LINE coverage 86.9%）、V03=PASS、V04=PASS、V05=PASS、V06=PASS、V07=PASS、V08a=PASS、V08b=PASS；`Verdict: ✅ all CRITICAL passed; exit=0`。
