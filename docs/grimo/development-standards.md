...
- **Spring AI Manual Configuration** — Never mix auto-config and Manual Config. Base config must declare `spring.ai.model.chat: none`, `spring.ai.model.embedding.text: none`, and `spring.ai.chat.client.enabled: false`. Build `ChatModel`, named `ChatClient`, and `EmbeddingModel` beans explicitly in `shared.ai.AiModelConfig`; `shared.ai` is exposed as Modulith `shared :: ai` for modules that need the compatibility factories. Provider builders (`com.google.genai.Client`, `GoogleGenAiChatModel`, `GoogleGenAiTextEmbeddingModel`) stay only in that config. Runtime classes consume Spring AI interfaces (`ChatClient` / `ChatModel` / `EmbeddingModel`) and must not import Google provider classes. Do not gate api-key-dependent model beans with build-time `@ConditionalOnProperty`; branch inside the factory method so AOT/native images see the bean definition and runtime Secret Manager values decide the path.
- **Spring AI structured output native hints (S173)** — Any production code that adds or changes `ChatClient.entity(Record.class)` or `new BeanOutputConverter<>(Record.class)` must update the module native config with `@RegisterReflectionForBinding(...)` for that record and its nested record/classes. The same PR must include a local guard test: either module-level reflection assertion like `ScanNativeConfigTest`, or the repo-wide `StructuredOutputNativeHintCoverageTest` must see the new target and pass. JVM-only JSON roundtrip tests do not protect native images because JVM record reflection works without GraalVM metadata. If the structured output runs in an outbox listener, async listener, scheduled job, or other background path, add a test where the analyzer/handler throws `Error` and verify the caller logs it, returns empty/noop output, and does not leave the event retrying forever.
