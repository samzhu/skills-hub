...
本 tick 讀 `shipping-release` skill 後做 preflight。`shipping-release` 的 guard 要求 `git status` 對 unrelated changes 乾淨，否則不能做 archive、changelog、roadmap release commit。
...
結果：BLOCKED。S170 自己已經 `verify-all.sh` 全綠，但 release 需要改 `docs/grimo/specs/spec-roadmap.md`、`docs/grimo/CHANGELOG.md`，並把本 spec 移到 `docs/grimo/specs/archive/`。目前 roadmap 已有非本 tick 未提交內容；若直接 ship，容易把 S147 / automation artifact 一起包進 S170 release commit。
