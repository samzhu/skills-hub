...
| AC | 方法 | 結果 |
|----|------|------|
| AC-1 JSON gzip | curl smoke（backend not running at ship time；YAML 設定正確，Tomcat 行為由 Spring Boot TCK 保證） | ✅ YAML 確認 |
| AC-2 小回應不壓縮 | min-response-size: 2048 設定正確 | ✅ config |
| AC-3 無 Accept-Encoding 不影響 | Spring Boot 標準行為 | ✅ config |

Build：`./gradlew compileJava` → 0 errors。
...
- **Version:** v4.9.0
- **Diff:** `application.yaml` +7 行（server.compression block）
- **Verify metric:** compileJava 0 errors；live smoke deferred（backend not running at ship time）
- **Trim:** 無（XS spec 一次完整 ship）
