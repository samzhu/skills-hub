...
| AC-8 manual smoke（curl `/oauth2/authorization/skillshub` 302 + 瀏覽器 Google 登入閉環） | ✅ PASS — user 於 LAB Cloud Run 跑完 `temp/DEPLOY-LAB-PRIVATE-IP.md` Step 14（Secret Manager 注入 client-id/secret + redirect-uri override + 瀏覽器 Google 登入回 `/skills` claims 正確） |
...
| AC-8：LAB Cloud Run E2E Google login flow | ✅ | user 完成 LAB 部署 + 瀏覽器 Google 登入閉環；F-4 redirect-uri override + F-8 AOT profile fix 都已 settled |
...
無 — 所有 AC 已 PASS（AC-8 LAB Cloud Run smoke 由 user 於 2026-05-07 完成驗證）。
