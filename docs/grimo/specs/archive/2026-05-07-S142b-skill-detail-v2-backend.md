...
> **Status**: ✅ shipped (v4.1.0 — 2026-05-07)
...
```
./gradlew test --tests "io.github.samzhu.skillshub.security.SecurityCategoryMapperTest" \
               --tests "io.github.samzhu.skillshub.security.SecurityReportControllerTest" \
               --tests "io.github.samzhu.skillshub.score.SkillScoreCalculatorTest" \
               --tests "io.github.samzhu.skillshub.score.QualityScoreControllerTest" \
               --tests "io.github.samzhu.skillshub.skill.query.SkillQueryServiceTest" \
               --tests "io.github.samzhu.skillshub.skill.query.SkillQueryControllerApiContractTest" \
               -x processTestAot
→ BUILD SUCCESSFUL (41 tests, 0 failures, 0 skipped)
```

Full suite (`./gradlew test -x processTestAot`): 602 tests, 88 pre-existing failures (ModularityTests `shared→skill` cycle + Testcontainers DB tests; all pre-date S142b). S142b-specific tests: **all green**.

### AC Coverage

| AC | Task | Outcome |
|----|------|---------|
| AC-S142b-1 (Skill 6 fields — PUBLISHED) | T04 | ✅ 4/4 SkillQueryServiceTest PASS |
| AC-S142b-2 (Verified DRAFT = false) | T04 | ✅ |
| AC-S142b-3 (Verified SUSPENDED = false) | T04 | ✅ |
| AC-S142b-4 (skillScore composite in /scores) | T03 | ✅ 6/6 SkillScoreCalculatorTest + 5/5 QualityScoreControllerTest PASS |
| AC-S142b-5 (Quality 404 → no skillScore) | T03 | ✅ protected by existing 404 path |
| AC-S142b-6 (SecurityReport 4-quad endpoint) | T02 | ✅ 3/3 SecurityReportControllerTest PASS |
| AC-S142b-7 (detail string format) | T01 | ✅ 21/21 SecurityCategoryMapperTest PASS |
| AC-S142b-8 (DoS 折入 shell) | T01 | ✅ |
| AC-S142b-9 (PI/META/LLM 不入 4-quad) | T01 | ✅ |
| AC-S142b-10 (404 SECURITY_NOT_SCANNED) | T02 | ✅ |
| AC-S142b-11 (API contract no breaking) | T04 | ✅ 2/2 SkillQueryControllerApiContractTest PASS |

### Trim / Deferred

- `SecurityReportServiceTest` (`@DataJdbcTest`) — deferred; covered by `SecurityReportControllerTest` `@WebMvcTest` slice + `SecurityCategoryMapperTest` unit
- `versionCount` computed via `List.size()` (loads all versions; < 50ms for typical skill); COUNT SQL optimisation deferred to follow-up
- `SkillQueryController.getByAuthorAndName()` also enriched (no extra test; symmetry with `findById`)

### New Files

| File | Lines | Purpose |
|------|-------|---------|
| `security/SecurityCategoryMapper.java` | ~130 | 4-quad partition + scoring |
| `security/SecurityReportService.java` | ~70 | JSONB → response projection |
| `security/SecurityReportController.java` | ~30 | GET /security-report |
| `security/SecurityReportResponse.java` | ~20 | Response DTO |
| `shared/api/SecurityNotScannedException.java` | ~10 | 404 path |
| `score/SkillScoreCalculator.java` | ~60 | composite compute + skillId lookup |
| `security/SecurityCategoryMapperTest.java` | ~150 | 21 unit tests |
| `security/SecurityReportControllerTest.java` | ~90 | 3 WebMvc slice tests |
| `score/SkillScoreCalculatorTest.java` | ~60 | 6 unit tests |
| `skill/query/SkillQueryServiceTest.java` | ~110 | 4 unit tests |
