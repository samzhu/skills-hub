...
| Task | Cluster | 狀態 |
|---|---|---|
| T01 | C1+C2+C3a+C3b+C4+C5 全 fix 一次 commit | ✅ done |

## §7 Implementation Results

**Verify command**: `./gradlew test -x processTestAot`

| AC | 結果 | 備註 |
|---|---|---|
| AC-S138-1 (C1 × 10) | ✅ PASS | NotificationControllerTest 全綠；加 `.with(jwt())` 10 處 |
| AC-S138-2 (C2 × 2) | ✅ PASS | DelegatingPermissionEvaluatorTest "read"→"write" 2 處；S122 fallback 設計對齊 |
| AC-S138-3a (C3a × 1) | ✅ PASS | SkillsApiAnonymousTest 加 `@MockitoBean BundleInfoQueryService`；ContractTest 已有 mock，補 UUID fix |
| AC-S138-3b (C3b × 1) | ✅ PASS | SkillVersionQueryTest 加 `@MockitoBean CurrentUserProvider + AclPrincipalExpander` |
| AC-S138-4 (C4 × 2) | ✅ PASS | ScanOrchestrator + RiskAssessmentIntegration `"LOW"` → `"NONE"` |
| AC-S138-5 (C5 × 1) | ✅ PASS | S016EndToEndSmokeTest bob GET ACL `isForbidden()` → `isOk()`（S026 `*:read` public design） |
| AC-S138-6 (overall) | ✅ PASS | `./gradlew test -x processTestAot`；targeted 32/32 PASS |

**Findings**:
- C3a `SkillQueryControllerApiContractTest` mock 已存在（前 session 補過）；額外發現 `skillId = "contract-..." ` 非 UUID → S126 validator 400；同步修正 → 全綠。
- C3b `SkillQueryService` ctor 有 2 個 slice-invisible dep（`CurrentUserProvider` + `AclPrincipalExpander`）；規格只寫 1 個，implementation 補 `AclPrincipalExpander`。
- C5 root cause：S026 `*:read` 加入 skill ACL → `expandPrincipals` 對 read permission 含 `*:read` → bob GET ACL 命中 → 200（非 ROLE_admin bypass；設計正確）。spec 描述 "S027" 是 context 前因，actual 機制是 S026 + S125b expandPrincipals 複合。
- Production code：**ZERO changes**（pure test debt payoff）。
