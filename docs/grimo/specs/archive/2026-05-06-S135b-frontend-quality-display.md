...
> Spec: S135b | Size: S(9-11) | Status: ✅ shipped v4.1.0
> Date: 2026-05-06
...
| # | Task | AC | Status |
|---|------|----|--------|
| T01 | api/scores.ts + hooks/useSkillScores.ts | — | PASS |
| T02 | QualitySection + QualityTab components | AC-1/2/3/5 | PASS |
| T03 | SkillDetailPage 整合（QualitySection + 品質 tab） | AC-1/2 | PASS |
| T04 | 測試：QualitySection.test / QualityTab.test | — | PASS |

Trim：QualityScorePill（SkillCard）defer → 後續 polish backlog（N cards = N API calls，需評估 prefetch 策略）
...
| Gate | Result |
|------|--------|
| `npx vitest run QualitySection.test QualityTab.test` | ✅ 2 files / 10 tests pass |
| `SkillDetailPage.test.tsx` regression | ✅ 7 tests pass |
| `npx tsc --noEmit` | ✅ clean |
| `npm run build` | ✅ 682 KB / gzip 190 KB（+0 KB 相比 v4.0.0 ~same baseline） |

### AC Results

| AC | Status | Evidence |
|----|--------|----------|
| AC-1: hero 顯示品質進度條 + 安全等級 | ✅ | QualitySection 3 狀態 test + build |
| AC-2: 品質 tab 顯示 3-axis 明細 + reasoning | ✅ | QualityTab test 覆蓋 axis label + dimension + totalScore |
| AC-3: 未評分 fallback 顯示 | ✅ | QualitySection null test + QualityTab null test |
| AC-4: SkillCard score pill | ⏸ deferred | N-card N-API call 問題，移至 polish backlog |
| AC-5: loading skeleton 不 layout shift | ✅ | QualitySection / QualityTab animate-pulse skeleton |

### 關鍵 Findings

#### F-1: AC-4 QualityScorePill defer 理由
SkillCard 是 list component（HomPage 每頁 20 cards）。self-fetching pill = 每 render 20 parallel GET /scores call。
短期對 dev 沒問題，但 prod 流量不友善。正確方式是 list API 回傳 totalScore 欄位或 batch endpoint。
決策：defer AC-4，後續 spec 處理（可能是加 GET /skills/{id}/scores mini-endpoint cache，或 list API 加 quality_score 欄位投影）。

#### F-2: scores hook 在 SkillDetailPage 整合
id 從 `skill?.id ?? params.id ?? ''` 取，確保雙路由（/skills/:id + /skills/:author/:name）都正確傳入 skillId。
空字串不觸發 fetch（`enabled: !!skillId`）。
