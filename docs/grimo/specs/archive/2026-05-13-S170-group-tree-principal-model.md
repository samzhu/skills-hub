...
> Spec: S170 | Size: M(16) | Status: ✅ shipped (v4.56.0)
> Date: 2026-05-13
...
### 7.10 Shipping Preflight — BLOCKED by unrelated working tree changes (2026-05-14)

本 tick 讀 `shipping-release` skill 後做 preflight。
...
```text
 M docs/grimo/specs/2026-05-08-S147-scanner-semantic-gap-research.md
 M docs/grimo/specs/spec-roadmap.md
?? .codex/
?? docs/grimo/tasks/
```

結果：BLOCKED。S170 自己已經 `verify-all.sh` 全綠，但 release 需要改 `docs/grimo/specs/spec-roadmap.md`、`docs/grimo/CHANGELOG.md`，並把本 spec 移到 `docs/grimo/specs/archive/`。

### 7.11 Full ship sync（2026-05-14）

完成 `shipping-release` 前置文件同步（未變更程式碼）：

- 將 spec 檔移到 `docs/grimo/specs/archive/2026-05-13-S170-group-tree-principal-model.md`。
- `docs/grimo/specs/spec-roadmap.md`：S170 變更為 shipped v4.56.0 並標註 S169 依賴已完成；新增 Milestone v4.56.0。
- `docs/grimo/CHANGELOG.md`：新增 v4.56.0 項目，重點列出 Group tree / principal context 行為、verify-all 結果。

`shipping-release` Guard 在本工作目標 tick 中不再阻檔，下一步請直接切進 release 流程。

下一步：先讓工作樹回到可切分狀態（例如把 S147/roadmap/.codex/tasks 另行提交、移到對應 spec commit，或由使用者確認可包含哪些 hunks），再重跑 `./scripts/verify-all.sh` 並執行 `shipping-release`。
