...
- `npm test src/pages/VersionDiffPage.test.tsx` → 5 passed (AC-1, AC-2, AC-3-S098c3, AC-3-legacy, AC-4)
- AC-3-S098c3 確認：mock `/file-list-diff` response → heading 顯示「+1 / -0 / ~0」+ `scripts/run.sh` 條目渲染

---

## §7 Result

- **backend** 新增 `FileListDiffResponse.java`（DTO + inner `FileDiffEntry`）、`SkillFileDiffService.java`（`listEntries(byte[])` ZipInputStream + `listDiff()`）
- **controller** `SkillQueryController` 加 `GET /skills/{id}/file-list-diff` endpoint（constructor injection）
- **frontend** 新增 `useFileListDiff.ts` hook + `api/skills.ts` 加 `FileListDiffResult / FileDiffEntry` 型別 + `fetchFileListDiff()`
- **VersionDiffPage** 加 `FileListDiffPanel`（綠 + / 紅 - / 橘 ~ 符號 + path + size label）；heading 顯示動態計數
- size 作為 modified 判斷（false negative 可接受 MVP）；per-file 行級 diff defer → S098c3b
- 全 5 frontend tests pass；backend compile clean
