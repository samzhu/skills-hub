...
> 規格：S197 | 大小：S(10) | 狀態：✅ shipped v4.80.0
> 日期：2026-05-18
...
狀態：shipped v4.80.0。S197-T05 已補上 `/publish` file mode page-level test，`./scripts/verify-all.sh` release tick 全綠。
...
S197 已由 `$shipping-release S197` 歸檔，S197 task file 已刪除，changelog / roadmap 已更新，release tag：`v4.80.0`。
...
### 7.7 Release Verification（2026-05-19）
...
PASS — V01=PASS V02=INFO（LINE coverage 87.2%，covered=4864 / total=5580）V03=PASS V04=PASS V05=PASS V06=PASS V07=PASS V08a=PASS V08b=PASS；`Verdict: ✅ all CRITICAL passed; exit=0`。

### Final Size Re-score (per estimation-scale.md)

| Dimension | Initial | Actual | Rationale |
|---|---:|---:|---|
| Tech risk | 1 | 1 | 只串接既有 React form state、`FileDropZone` props、RTL tests；沒有新增套件、後端 API、schema migration 或 browser-only API。 |
| Uncertainty | 1 | 2 | QA re-review 前才抓到 `/publish` page-level file mode 沒把 required error 傳進 `FileDropZone`，因此追加 S197-T05。 |
| Dependencies | 1 | 1 | 仍只依賴已 shipped 的 S176、S188、S195；沒有新增 cross-spec 前置。 |
| Scope | 2 | 2 | 實際改動維持在 Publish/Edit/FileDropZone 與 DESIGN/test 文件，但比原計畫多一個 QA fix task。 |
| Testing | 2 | 3 | 除三個 targeted frontend test 檔 48 tests 外，shipping gate 又重跑 `./scripts/verify-all.sh`，包含 backend、frontend、Playwright happy-path、AOT 與 `bootBuildImage`。 |
| Reversibility | 1 | 1 | 只改前端顯示與文件；不變更資料結構或 API contract，回退成本低。 |
| **Total** | **8 / S** | **10 / S** | Bucket 不變；主要差異是 QA fix 補上 `/publish` 實際頁面 contract 與完整 release gate 證據。 |
