...
POC：not required — S196 是 frontend-only UI rework，沿用既有 `useRequests({ sort })`、`createRequest`、`VoteButton`、`AuthGatedButton`、TanStack Query invalidation 與 React Testing Library；沒有新 package、SDK、framework SPI、DB migration 或外部 API 假設需要先驗證。

| 順序 | Task | 檔案 | AC | 驗證 |
|---|---|---|---|---|
| 1 | `S196-T01` Request Board browse tabs and ranking | `docs/grimo/tasks/2026-05-18-S196-T01-request-board-browse-tabs.md` | AC-S196-1, AC-S196-2, AC-S196-4, AC-S196-5, AC-S196-6 | `cd frontend && npm test -- RequestBoardPage` |
| 2 | `S196-T02` Inline request creation panel and design sync | `docs/grimo/tasks/2026-05-18-S196-T02-inline-create-panel-design-sync.md` | AC-S196-3, AC-S196-7, AC-S196-6 final check | `cd frontend && npm test -- RequestBoardPage && npm run verify` |

Execution notes:

- `S196-T01` must establish the tab shell first, because `S196-T02` mounts the inline form inside the `create` tab panel.
- `S196-T02` may delete `CreateRequestModal.tsx` only after confirming no other import remains.
- Browser E2E is not required for task planning because this spec does not cross a backend or browser-only artifact seam; component tests can verify the tab contract, request URLs, POST body, and DESIGN/prototype source text.
