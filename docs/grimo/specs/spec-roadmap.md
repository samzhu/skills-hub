...
| Phase 5 | M116: SkillQueryService.search() row-level ACL filter (Bug AS fix — **LAB-blocker**) | S121 | S(4-5) | — | 📋 planned — Mode B Round 37 (2026-05-04) finding **CRITICAL**。`SkillQueryService.search()` line 133-138 SQL 完全沒套 acl_entries `?\|` filter — 只 `WHERE status='PUBLISHED'`；S016 ship 的 `SkillPermissionStrategy` 只給 `@PreAuthorize` 用，list endpoint 路徑從未套用。**S116 visibility toggle 在 list endpoint 完全失效**（PRIVATE skill 仍 visible 給 anonymous + non-granted user）。`SkillshubPgVectorStore.searchByEmbeddingWithAcl` (S017 line 136) 有 filter 但只 semantic search 路徑走；keyword search / 主 list 路徑全裸。E2E 驗證（OAuth=true mode）：anonymous list `total=2`（含 PRIVATE skill；應只 1 PUBLIC）。Fix：SQL 加 `AND acl_entries ??\| :patterns::text[]` clause；inject `AclPrincipalExpander` + `CurrentUserProvider`；author-mode 維持 S094a 行為（owner 看自己 DRAFT/SUSPENDED 不套 ACL）；對應 countSql 同步加。Test：anonymous list 看 PRIVATE 應 0；B grant 後 list 應 +1。Single-tick S=4-5。**LAB 封測前必補**否則 visibility 設定全失效。 |
| Phase 5 | M117: SkillQueryController.getById @PreAuthorize (Bug AT fix) | S122 | XS(2) | — | 📋 planned — Mode B Round 37 finding HIGH。Single GET endpoint 缺 ACL 守則；anonymous 直打 `/api/v1/skills/{private-id}` 拿到完整 JSON（per S114a plan 已知 gap，本 round 端到端確認 OAuth=true mode 仍 leak）。Fix：加 `@PreAuthorize("hasPermission(#id, 'Skill', 'read')")` + 對應 GlobalExceptionHandler AccessDenied → 403 mapping（既驗）。Single-tick XS。對齊 S121 list path enforcement，補單筆 path。 |
