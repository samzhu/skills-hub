...
| Phase 4 | M90h2: Notifications full projection (Version Diff 剝離至 S098c2/c3) | S096h2 | M(12) re-est from M(10-12) | — | 📐 in-design — 2026-05-03 spec doc 寫成。新建 `notification` 模組（與 audit/analytics 同層）；CQRS event-driven projection — 4 個 `@ApplicationModuleListener` 訂閱 SkillFlagged / ReviewCreated / RequestClaimed / RequestFulfilled events；計算 recipient (skill.owner_id / request.requester_id) → INSERT notifications 表；UNIQUE(recipient_id, ref_event_id, category) 防 outbox redelivery 重複。新加 notification_preferences 表 (4 categories on/off)。6 endpoints：GET list (Slice cursor) / GET unread-count / GET preferences / POST mark-read / POST read-all / POST update-preferences / DELETE。14 ACs。Polling 30s MVP（既有機制不破壞）；SSE/WebSocket defer。Trigger rules MVP 3 條 self-direct（broadcast / vote-followers / version-followers defer）。Version Diff scope 從 row 剝離 — 由 📋 S098c2 (M8) + S098c3 (L12) 處理。 |
