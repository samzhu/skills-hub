...
| Phase 5 | M119: getByAuthorAndName ACL gate (S122/S123 follow-up) | S124 | XS(2-3) | — | 📋 planned — S122/S123 ship 後 read-side ACL chain 唯一漏 endpoint：`GET /skills/{author}/{name}` (canonical alias path per S096c) 不同 SpEL signature（兩 path variable 而非單一 id）。需設計 hasPermission 評估前先 resolve(author, name) → id，或新 PermissionStrategy `byAuthorAndName(author, name, permission)` overload。非 LAB 封測 critical（dev/frontend 主要走 `/{id}` path），可 LAB 部署後補。 |
