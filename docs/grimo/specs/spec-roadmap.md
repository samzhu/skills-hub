...
| Phase 5 | M120b: SkillSubscription endpoints + version-published listener (S125 split 2/3) | S125b | XS(3) | — | 📋 planned — S125a 後接續。Add：(1) `POST /api/v1/skills/{id}/subscribe` (201) + `DELETE /api/v1/skills/{id}/subscribe` (204) + `GET /api/v1/me/subscriptions` (list)；@PreAuthorize 守 `read` permission（避免 anon subscribe）。(2) `NotificationProjectionListener.onVersionPublished` 訂閱 `SkillVersionPublishedEvent` → 從 `SkillSubscriptionService.findSubscribersOf(skillId)` 查 subscriber list → 對每個 subscriber 寫 notification（category=`versions`，title=「{skill.name} {version} 已發布」+ self-action skip + UNIQUE(recipient_id, ref_event_id, category) idempotency）。對齊 NotificationProjectionListener 既驗 4 listener pattern。LAB 封測前必 ship 補完 user-visible flow。 |
