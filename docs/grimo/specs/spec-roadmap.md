...
| Phase 4 | M92e2: Reviews aggregate + ratings + SkillDetail Reviews tab | S098e2 | S(11) re-est from M(8) | — | 🚧 in-progress 1/4 — T01 backend aggregate + endpoints shipped Tick 8（review/ 模組 + Review aggregate + ReviewService + 合併 controller + V8 migration + ReviewServiceTest 8/8 PASS @ 17.2s + ModularityTests 仍乾淨；deviation：delete flow 改 ApplicationEventPublisher 直接發 event 因 state-based aggregate 無 @Version 不適合 save() 觸發 outbox）。Pending：T02 projection listener + skill 加欄位 → T03 frontend infra → T04 SkillDetailPage Reviews tab。原始 design：2026-05-03 spec doc 寫成；新建 `review` 模組（ADR-002 canonical pattern：Spring Data JDBC 充血聚合 + Modulith Outbox + AFTER_COMMIT projection）；3 endpoints (POST/GET/DELETE)；2 schema migrations（reviews 表 + skills 加 average_rating/review_count）；12 ACs (8 backend + 4 frontend：含 1-5 星 / 1 則 per user / hero+list UI / 提交 form happy path)；6 個產品決策定案（1-5 星、1-per-user、強制登入、無 helpful、Approach A 投影、無 moderation）。Listener 放置位置 (review-side vs skill-side) 由 implementer 視 Modulith verifier 決定（S112-T01 deviation 啟示）。Hypothesis 不阻塞 spec，無需 POC。為 S101b Impact Score 提供 rating sub-metric。 |
| Phase 4 | M92e3: Flag 回報流程 — POST form + reviewer queue + status mutations | S098e3 | S(8) re-est from M(7) | — | 📐 in-design — 2026-05-03 spec doc 寫成；補完 ✅ S112 (read 端) 對應的 write + reviewer 端。零 schema migration（status 既是 String 欄位，純 enum 擴充 OPEN→RESOLVED/DISMISSED）；4 backend changes (FlagController PATCH + cross-skill GET /flags + createFlag 加 reporter 參數 + FlagStatus enum)；3 frontend additions (FlagSubmitModal + /flags reviewer queue page + AppShell nav)；13 ACs (8 backend + 5 frontend)；7 個 UX 決策（Feature First：任何登入用戶可看 reviewer queue；OPEN→RESOLVED/DISMISSED 雙 terminal；reportedBy 改 useMe.sub；submission Modal）。對齊 PATCH semantic for status update (Approach A)。為 S112 read 端補完 write loop。 |
