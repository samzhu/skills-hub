...
| Phase 5 | M106: Flag wiring full-stack — SkillDetail Flags tab + MySkills openCount metric | S112 | S(7) | — | 📐 in-design — 2026-05-03 page audit fresh round 發現 backend `FlagController` 已完整 ship 但前端兩處未串：(1) SkillDetailPage Flags tab 永遠 EmptyState；(2) MySkillsPage 「待處理回報」MetricCard 寫死 `value={0}`。本 spec 接 GET /skills/{id}/flags + 新加 GET /me/flags-summary（aggregate count）+ 移除 MySkillsPage 「平均評分」假 metric（等 S101a Quality Score）。7 ACs，含 user-isolation + status-filter 行為驗證。與 📋 S098e3（reviewer queue + POST 表單）/ 📋 S098e2（Reviews aggregate）不衝突。 |
