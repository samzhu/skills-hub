...
| Phase 4 | M90d3: Publish flow restructure + new domain events + per-skill stats | S096d3 | M(10-12) | — | 📋 planned — split from S096d2; PublishPage 單頁 → /upload + /validate + /review three-step SSE; 3 new domain events (Bundle/Frontmatter/RiskScan); per-skill stats endpoint /api/v1/skills/{id}/stats?period=30d |
