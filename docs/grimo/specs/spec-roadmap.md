...
| Phase 4 | M90g2: Request Board full feature — aggregate + voting + claim + fulfillment | S096g2 | S(11) re-est from M(10-12) | — | 🚧 in-progress 1/4 — T01 backend aggregate + endpoints + V10 migration shipped Tick 18（community module 正式註冊 + Request aggregate ADR-002 充血 + 5 event records + RequestService 5 methods + RequestCommand/QueryController + 2 exceptions + V10 schema；RequestServiceTest 13/13 PASS @ 9.5s + ModularityTests 仍乾淨；deviation：用 @Query 而非 derived query 避開 Spring Data JDBC AOT codegen 多屬性 sort bug）。Pending：T02 vote toggle → T03 frontend infra → T04 RequestBoardPage + components。原始 design：ADR-002 canonical aggregate (Spring Data JDBC 充血 + Modulith Outbox)；7 endpoints；2 schema migrations (requests + request_votes UNIQUE join)；5 domain events；17 ACs (14 backend + 3 frontend)；7 個 UX 決策。Vote count 走 raw SQL atomic UPDATE pattern (對齊 S076 download_count)。為 S096h2 Notifications 提供 5 個 event hook。 |
