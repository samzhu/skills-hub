...
> 最後更新：2026-05-20（S205 in-design）
...
| S205 | Download filename UTF-8 Content-Disposition（中文顯示名稱下載檔名不再落成 `download`） | XS(8) | S061 ✅, S176 ✅, S188 ✅ | 📐 in-design — `Content-Disposition` 改用 RFC 5987 `filename*` UTF-8 encoding |
---
...
| v4.89.0 | S205 (Download Filename UTF-8 Content-Disposition) | 📐 in-design — `/api/v1/skills/{id}/download` 中文顯示名稱用 `filename*` UTF-8 encoding；修 `OAuth 專家` 下載檔名 fallback |
