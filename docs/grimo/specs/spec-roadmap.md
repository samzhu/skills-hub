...
|------|------|-------|--------|------|--------|
| 1 | S019 | JaCoCo coverage gate + 80% line threshold | XS(5) | — | 🔲 Planning |
| 2 | S020 | Verification command registry + `scripts/verify-all.sh` | S(10) | S019 | 🔲 Planning |
| 3 | S021 | Phase 2 doc-sync — PRD.md + architecture.md | S(8) | — (可與 1/2 平行) | 🔲 Planning |
...
| 6 | S018 | Skill Aggregate 充血演化 + Suspend/Reactivate Events | S(11) | S014 ✅ + S016 | ⏳ Design (paused) |

> **S018 注記**：spec 檔案已存在於 `docs/grimo/specs/2026-04-27-S018-skill-aggregate-rich-domain.md`（先前 design 階段產出）。S016 ship 後需回到 S018 design 重審 PermissionEvaluator 整合點，再進 `/planning-tasks`。
...
| **Phase 2** | **M16: Skill Aggregate 充血演化** | **S018** | **S(11)** | **225** | **⏳ Design (paused)** |
...
### Milestone 16: Skill Aggregate 充血演化 + Suspend/Reactivate ⏳ Design (paused)
...
| # | Spec | Points | Dependencies | Status |
|---|------|--------|--------------|--------|
| S018 | Skill Aggregate 充血演化 + Suspend/Reactivate Events | S(11) | S014 ✅（event store JDBC）+ S016（PermissionEvaluator） | ⏳ Design (paused) |
