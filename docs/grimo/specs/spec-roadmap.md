...
| Phase 5 | M110: JWT + ACL Safety Design — schema evolution + graceful degradation | S115 | M(8-10) | — | 📐 in-design — 2026-05-03 spec doc 寫成（user mid-tick 直接 directive 接手；spec + ADR-006 雙寫）。Core invariant：sub 為唯一 REQUIRED claim（缺 → 401 不走 fallback）；其他 claim 全 optional 走「安全保守值 + WARN log + Micrometer counter」(roles/groups → empty list；non-string elements → skip per element)。CurrentUserProvider 補 sub null check（既有 `jwt.getName()` NPE 風險）+ `parseStringListClaim` helper graceful 型別錯路徑。新加 MissingJwtSubException + GlobalExceptionHandler 路由 401 + WWW-Authenticate header。AC-8 端到端守 prod (oauth.enabled=true) 模式不誤打 fallback branch 3 (labUserId/admin)。12 ACs（含 forward-compat 未知 claim test + ACL principal types 安全 invariant test + ADR review checklist）。Depends S114a 為 `company` principal type baseline；可 parallel ship。 |
