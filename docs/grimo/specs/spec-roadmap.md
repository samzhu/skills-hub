...
| Phase 5 | M101: Semantic search response projection completeness | S107 | S(5-7) | — | 📋 planned — Mode B Round 12 (Chrome MCP semantic search live + backend curl 對比) audit: backend `/api/v1/search/semantic` response 缺 author / category / riskLevel 欄位（全 empty/null），同 skill `/skills/{id}` 返回完整資料；前端 unsafe cast 隱藏 type 警告，造成 `/search` 全顯「未評估」+ 缺 author/category。Backend DTO + projection fix；前端可選 cast removal polish。寫於 2026-05-03 cron Tick 12 (Spec-Only-Handoff). |
