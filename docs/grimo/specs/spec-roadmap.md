...
| Phase 5 | M122: MissingPathVariableException ErrorResponse 一致性 (Bug AY fix) | S127 | XS(1) | — | 📋 planned — Mode B Round 39 finding LOW。`/skills//foo`（空 author path variable）走 Spring 預設 400 ErrorResponse shape（`status:400, error:"Bad Request"`）而非 GlobalExceptionHandler 標準 ErrorResponse（`error:"VALIDATION_ERROR"`）。低衝擊（frontend 不會生此 URL）。Fix：GlobalExceptionHandler 加 `@ExceptionHandler(MissingPathVariableException.class)` + `@ExceptionHandler(NoResourceFoundException.class)` 翻譯為標準 shape。Single-tick XS=1。 |
