...
> 最後更新：2026-05-21（S204 V07b targeted fix PASS；next $verifying-quality S204 full release gate）
...
| S204 | OAuth Login Error Page（OAuth callback 失敗導到 `/auth/error`，不再顯示 Spring default `/login?error`） | S(5) | S139 ✅, S152 ✅ | ⏳ QA recheck pending — T01/T02/T03 PASS；V07b targeted fix PASS；next `$verifying-quality S204` full release gate |
...
| v4.88.0 | S204 (OAuth Login Error Page) | ⏳ QA recheck pending — T01/T02/T03 PASS；V07b targeted fix PASS；next `$verifying-quality S204` full release gate |
