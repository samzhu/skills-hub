...
| Phase 4 | M92a: Publish Step 2 `/publish/validate` page (poll-based) | S098a | M(10) | — | 📋 planned — prototype #5 4-step stepper UI; bridges Step 1 upload → Step 3 review |
...
| Phase 4 | M92b3: Validation breakdown UI (full) | S098b3 | S(5) split from S098b | — | 📋 planned — prototype #7 v-section / err-row / stepper full structure；目前 msg 只簡單 pre-block |
| Phase 4 | M92c: Version Diff page + backend endpoint | S098c | M(12) | — | 📋 planned — `/skills/:author/:name/diff?from=&to=`; new GET /api/v1/skills/{id}/diff; prototype #12 side-by-side |
...
| Phase 4 | M92d2: Homepage risk filter sidebar | S098d2 | S(5) split from S098d | — | 📋 planned — risk-tier sidebar with count breakdown；新 endpoint `/skills/risk-counts` 或 client-side aggregation；prototype #2 sidebar |
...
| Phase 4 | M92e2: Reviews aggregate + ratings | S098e2 | M(8) split from S098e | — | 📋 planned — backend Review aggregate + endpoints + frontend submission UI |
...
| Phase 4 | M92f2: Remaining 3 doc pages stubs | S098f2 | S(6) split from S098f | — | 📋 planned — SKILL.md spec / Frontmatter fields / Bundle structure 3 個 reference 詳細 walkthrough |
| Phase 4 | M92f3: Publishing + API/Webhook docs groups | S098f3 | M(8) split from S098f | — | 📋 planned — 上傳與驗證 / 版本管理 / 語意搜尋 / REST 參考 / Event payload 5 doc pages |
