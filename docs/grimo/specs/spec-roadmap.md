...
> 最後更新：2026-05-19（S200 local implementation PASS；next `$verifying-quality S200`）
...
| S200 | Request requester display identity（需求詳情 header 不顯 `u_<id>`；request DTO 補 requesterDisplayName/requesterHandle） | XS(4) | S192 ✅, S156c ✅, S196 ✅ | ⏳ QA — S200-T01/T02 PASS；local implementation PASS；next `$verifying-quality S200` |
