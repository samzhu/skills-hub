...
| Phase 5 | M111: Skill Visibility Toggle — public / private at create | S116 | S(8) | — | 📐 in-design — 2026-05-03 spec doc 寫成（user mid-tick directive 2/2 完成 spec planning）。Approach B：走 derived from `acl_entries` 是否含 `*:read`（不擴 schema；對齊 S038 既驗 listEntries 識別 *:read）。新加 `Visibility` enum (PUBLIC/PRIVATE) + `CreateSkillCommand.visibility` field + 4-arg backward-compat ctor delegate to 5-arg with PUBLIC default。`Skill.create` factory 條件式 seed `*:read`：PUBLIC 加；PRIVATE 不加（無 author + PRIVATE → IllegalArgumentException）。SkillCommandController POST 加 `@RequestParam visibility=PUBLIC` default；frontend PublishPage 加 radio group fieldset + state + uploadSkill 5-arg。9 ACs（含 backward-compat 4-arg + anonymous filter on private 既驗 path 無改動 + private owner read）。SkillCreatedEvent 不擴（defer event field 避 caller migration cost）。Defer: visibility 變更後操作 (post-create migration UX) / SkillDetail visibility badge / MySkillsPage visibility filter chip / explicit `is_public` column (S114a 已 plan)。Depends S114a 為 future `is_public` GENERATED column；MVP 不 block S114a。 |
