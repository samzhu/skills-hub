...
| Phase 4 | M90h2: Notifications full projection (Version Diff 剝離至 S098c2/c3) | S096h2 | M(12) re-est from M(10-12) | — | 🚧 task-planned (Tick 22 拆 4 BDD task files；T01 backend aggregates+V11 schema+module wiring → T02 NotificationProjectionListener 4 個 @ApplicationModuleListener 含 idempotency → T03 mutation/query services + controller 取代 stub → T04 frontend api/notifications.ts split + 2 hooks + NotificationsPage 改寫 + PreferencesModal)。原始 design：CQRS event-driven projection — 4 個 listener 訂閱 SkillFlagged / ReviewCreated / RequestClaimed / RequestFulfilled events；計算 recipient (skill.owner_id / request.requester_id) → INSERT notifications 表；UNIQUE(recipient_id, ref_event_id, category) 防 outbox redelivery 重複。新加 notification_preferences 表 (4 categories on/off)。6 endpoints。14 ACs。Polling 30s MVP；SSE/WebSocket defer。Trigger rules MVP 3 條 self-direct。 |
