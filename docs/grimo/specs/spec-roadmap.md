...
| Phase 5 | M111: Skill Visibility Toggle — public / private at create | S116 | S(8) | — | 📋 planned — user 2026-05-03 mid-tick 直接需求：新增 skill 時可選 `public`（全開）或 `private`（私人，自己再共享給別人）— GitHub repo visibility model。對齊既有 `*:read` synthetic public principal (S026 / S038)：public skill = 自動含 `*:read` ACL entry；private skill = 預設只 owner + 顯式 grant。UI：PublishPage / SkillCommandService 新增 visibility radio group + tooltip。Defer：visibility migration after publish（owner 改 public ↔ private）— 留 polish。Depends S114a 為 ACL projection 後正式 visibility column；MVP 可走既有 `acl_entries` JSONB path 不 block S114a。可 parallel S115 ship。 |
