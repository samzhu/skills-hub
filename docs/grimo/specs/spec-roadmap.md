...
| Phase 4 | M90g2: Request Board full feature — aggregate + voting + claim + fulfillment | S096g2 | S(11) re-est from M(10-12) | — | 🚧 in-progress 2/4 — T01 backend aggregate (Tick 18) + T02 vote toggle (Tick 19：RequestVoteService atomic SQL INSERT ON CONFLICT + UPDATE GREATEST guard + RequestVotedEvent publish；POST /vote endpoint；RequestVoteServiceTest 5/5 PASS @ 9.7s) shipped。Pending：T03 frontend infra → T04 RequestBoardPage + components。原始 design：ADR-002 canonical aggregate (Spring Data JDBC 充血 + Modulith Outbox)；7 endpoints；2 schema migrations (requests + request_votes UNIQUE join)；5 domain events；17 ACs (14 backend + 3 frontend)；7 個 UX 決策。Vote count 走 raw SQL atomic UPDATE pattern (對齊 S076 download_count)。為 S096h2 Notifications 提供 5 個 event hook。 |
