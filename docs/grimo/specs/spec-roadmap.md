...
| Phase 5 | M93a: OpenAPI 3.1 verification | S099a | XS(2) | — | 📋 planned — verify SpringDoc /v3/api-docs 為 3.1.0；應已是；若否加 application.yaml `springdoc.api-docs.version: openapi_3_1`。Docs page 加 OpenAPI version note。 |
...
| Phase 5 | M100: Sort `推薦` behavior alignment with design intent | S106 | XS(2) | — | 📋 planned — Mode B Round 11 (Chrome MCP click sort chips + compare first card) audit: 「推薦」與「最新」chip 行為相同（都 fall through backend default `created_at DESC`），UI 4 chip 但只 3 種行為。HomePage:17 comment stale (聲稱 backend default = downloadCount desc)；skills.ts:46 conditional 排除 'recommended' 不送 sort param。Frontend-only fix: 加 explicit `recommended → downloadCount,desc` mapping (per design intent)；同 most-downloaded 但 UI chip 仍 distinct，future recommendation algorithm 預留位。寫於 2026-05-03 cron Tick 10 (Spec-Only-Handoff). |
