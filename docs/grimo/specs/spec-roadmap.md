...
| Phase 4 | M92a2: SSE 事件串流 + per-step 動畫 | S098a2 | M(8) split from S098a | — | ⏸ deferred (post-MVP per user 2026-05-03) — polling 30s 既有機制（S098a / S096d5a ship）已 cover 80% UX；SSE 是「✅ 解壓 → ⏳ 掃描」打勾動畫 polish。觸發條件：user 抱怨「不知道要等多久」/ risk scan 平均 > 5s 才優先。Backend 3 events (BundleParsed/FrontmatterValidated/RiskScanCompleted) + frontend EventSource client。 |
...
| Phase 4 | M92a3-2: Backend bundle-info endpoint | S098a3-2 | XS(2) split from S098a3 | v3.8.1 | ✅ shipped 2026-05-03 cron Tick 31-32 (1 tick spec planning + 1 tick ship)。V13 migration `skill_versions.file_count INTEGER NOT NULL DEFAULT 0` + PackageService.countEntries helper + PublishVersionCommand 加 fileCount + SkillVersion @Column 同步 + 兩 upload pipeline (uploadSkill / addVersion) 計算 + BundleInfoQueryService + GET /skills/{id}/bundle-info endpoint + BundleNotPublishedException (404 distinct error code) + GlobalExceptionHandler。Frontend useBundleInfo hook (60s cache, retry:false) + BundleInfo type + PublishValidatePage upload-strip 顯實值（fileSize KB / fileCount 個檔案 / fileCount=0 hide 該欄）+ fallback derived placeholder for endpoint 404。Backend 18 個 regression tests + 6 個新 BundleInfoQueryServiceTest 全 PASS（含 6 個 cross-test PublishVersionCommand callsite migration：fileSize, Map → fileSize, fileCount, Map）；frontend 193/193 PASS @ 7.32s（0 regression）+ tsc PASS。對齊 useSkill / useRequest enabled-gate canonical 第 5 次採用。Defer: 既有 row backfill job (polish) / hover tooltip 精確 byte size / uploadedAt relative time formatter。 |
...
| Phase 4 | M90d6: /publish/validate Step 2 page + new domain events + SSE | S096d6 | M(8-10) split from S096d5 | — | ⏸ deferred (post-MVP per user 2026-05-03) — overlap S098a2 (SSE)；page 本身已由 S098a (poll-based) ship；3 個 publish pipeline events (BundleParsed/FrontmatterValidated/RiskScanCompleted) + SSE 全為 polish。MVP polling 夠用；SSE 觸發條件同 S098a2。整 spec deferred。 |
...
| Phase 4 | M90f2: Collections full feature | S096f2 | M(13) re-est from M(10-12) | v3.8.0 | ✅ shipped 2026-05-03 cron Tick 26-30 (1 tick spec planning + 1 tick T01 backend infra + 1 tick T02 service/controllers + 1 tick T03 FE infra + 1 tick T04 frontend full + spec ship)。T01 Collection aggregate (AbstractAggregateRoot + @Version + @MappedCollection skills) + V12 schema (collections + collection_skills CASCADE + UNIQUE skill_id) + community @ApplicationModule formal register + 2 events + SkillRepo `findAllByIdInAndStatus`；T02 CollectionService 業務邏輯 + Command/Query controllers split + 2 exception class (CollectionNotFound 404 + SkillNotPublishable 400) + GlobalExceptionHandler + RequestService.fulfill caller migration (`IllegalArgumentException → SkillNotPublishableException`)；T03 api/skills.ts 3 helper + 2 type + useCollections + useCollection hooks + CollectionsPage extract；T04 CreateCollectionModal (textarea skill picker per §2.6 trim) + InstallButton (50ms loop browser download trigger) + CollectionsPage rewrite + AC-10/11/12 tests。Backend 26 tests + frontend 4 tests + ModularityTests 2/2 全 PASS。Spec-Only-Handoff pattern 第 8 次 demo（4-tick implement loop avg）；@Version + AbstractAggregateRoot canonical 第 4 次採用；Command/Query split 第 2 次；Exception 獨立 class 升級 naming 第 5 次。Defer: risk filter (S096f3 polish 排隊) / 漂亮 multi-select skill picker (MVP 走 textarea UUID) / Collection detail page (用 modal 顯) / edit/delete。為 S101b Impact Score 提供 install event hook。 |
...
| Phase 4 | M90g2: Request Board full feature — aggregate + voting + claim + fulfillment | S096g2 | S(11) re-est from M(10-12) | v3.6.0 | ✅ shipped 2026-05-03 cron Tick 18-21 (4 ticks ship + 1 spec planning Tick 17) — T01 backend Request aggregate + 7 endpoints + V10 schema (Tick 18) → T02 vote toggle atomic SQL + race tests (Tick 19) → T03 frontend infra api/hooks (Tick 20) → T04 RequestBoardPage CTA + CreateRequestModal + VoteButton (樂觀更新) + RequestActionBar (state-aware) + AC-15/16/17 tests (Tick 21)。Backend 18 tests + frontend 3 tests 全 PASS；ModularityTests 仍乾淨（community 正式註冊 ApplicationModule + skill::domain/query NamedInterface SPI）。為 S096h2 Notifications 提供 5 個 event hook（RequestPosted/Voted/Claimed/Released/Fulfilled）。Spec-Only-Handoff pattern 第 6 次 demo。Defer：sort chips / status filter UI / fancy fulfill skill picker / votedByMe 列表欄位（per §2.4 trim list；後續 polish spec）。 |
...
| Phase 4 | M90h2: Notifications full projection (Version Diff 剝離至 S098c2/c3) | S096h2 | M(12) re-est from M(10-12) | v3.7.0 | ✅ shipped 2026-05-03 cron Tick 22-24 (1 tick spec planning + 2 ticks backend infra + 1 tick frontend full) — T01 backend aggregates (Notification + NotificationPreference @Version) + V11 schema (notifications + notification_preferences + UNIQUE(recipient_id, ref_event_id, category) idempotency) + module wiring (Tick 23) → T02 NotificationProjectionListener 4 個 @ApplicationModuleListener 跨 4 module SPI + 2 NamedInterface 暴露 events (community::events / review::domain) (Tick 23) → T03 mutation/query services + 取代 stub controller 7 endpoints + 2 exception 翻 404/403 + Slice cursor pagination (Tick 24) → T04 frontend api/notifications.ts split (7 helpers + 2 type) + 2 hooks (useNotifications + useNotificationPreferences) + PreferencesModal 4 toggle + NotificationsPage 全面 rewrite (filter chips + 全部已讀 + 設定 + mark-read row click + delete row ✕) + AppShell import 切換 + skills.ts 移除舊 Notification + 7 tests (AC-12/14)。Backend 22 tests + frontend 7 tests + ModularityTests 2/2 全 PASS。Spec-Only-Handoff pattern 第 7 次 demo。Defer：versions category 整類（listener 不產，spec §2.6 trim）/ broadcast notifications / SSE-WebSocket / digest grouping / cursor pagination UI / delete confirm（per §2.6 trim list；後續 polish）。 |
...
| Phase 5 | M107: RBAC ACL with materialized projection — Owner + Viewer roles | S114a | M(12) | — | 📐 in-design — 2026-05-03 spec doc 寫成（10M POC + 業界研究 + per-user role+async directive 整合）。CQRS-lite：`skill_grants` 表為 write side source-of-truth + `acl_entries` JSONB 變 async projection；新增 `skills.owner_id` column + `is_public` generated column + partial index（POC 驗 8x 加速最高頻 path）；新加 `company` principal type；既有 list / single GET 補 row-level ACL filter（修 ACL audit gap）；6 個 UX 決策（Owner+Viewer 2 role / 單 owner / uniform-via-grants visibility / async AFTER_COMMIT projection / S016 grantAcl deprecated 但 backward compat / V<n+1> migration backfill 既有 ACL）。12 ACs（10 backend + 2 frontend）。對標 Google Drive sharing model；ADR-005 同期新增整合既有 6 個 ACL spec。為 S114b/c 鋪路。 |
| Phase 5 | M108: ACL production scale path — Slice + Caffeine + pattern cap + metrics | S114b | M(10) | — | 📋 planned — 真實負載觀察後再 ship。Slice 取代 Page（避 COUNT 災難）+ Caffeine cache permission check（5min TTL 事件失效）+ pattern count cap (warn 30 / error 100) + Micrometer ACL metrics + slow query log。Depends S114a。 |
| Phase 5 | M109: ACL infra — read replica + PgBouncer | S114c | M(8) | — | 📋 planned — 真實負載觀察後再 ship。Cloud SQL read replica + PgBouncer transaction-mode pooling sidecar + Spring DataSource read/write routing。Infra-side spec parallel to S114b。 |
| Phase 5 | M110: JWT + ACL Safety Design — schema evolution + graceful degradation | S115 | M(8-10) trimmed | v3.8.2 | ✅ shipped 2026-05-03 cron Tick 33-34 (1 tick spec planning + 1 tick trimmed implement)。Core: CurrentUserProvider sub null check + parseStringListClaim helper graceful 型別錯路徑 + JwtClaimAnomalyMetrics Micrometer counter + MissingJwtSubException → 401 + WWW-Authenticate header + GlobalExceptionHandler mapping。CurrentUserProviderTest 11/11 PASS（既有 5 + 新加 6 AC-1/2/3/4/5：missing sub / blank sub / roles type mismatch / non-string element skip / groups null / unknown claim forward-compat）。ADR-006 ship 5-段範本 + 4 個 matrix 表（claim 必要性 / ACL principal types fail-closed / OAuth2 error → HTTP / Observability）+ Implementation snapshot。Defer (polish): AC-8 prod fallback guard test (@SpringBootTest setup heavy) / JwtSafetyTest E2E / JwtDecoder OIDC discovery graceful 503 / Grafana dashboard panel JSON / glossary + development-standards 更新。第 6 次 Exception 獨立 class 升級 naming canonical（MissingJwtSubException + 既有 5 個）；codebase 第 1 次採用 SimpleMeterRegistry test pattern。User mid-tick directive 1/2 完成。 |
| Phase 5 | M112: Frontend SkillVersion type sync `fileCount` (Bug AP fix) | S117 | XS(1) | v3.10.3 | ✅ shipped 2026-05-04 cron Tick 10 (single-tick ship)。SkillVersion interface 加 `fileCount: number` 對齊 backend (per S098a3-2 既驗 expose) + VersionList row graceful conditional `{fileCount > 0 && <span>{fileCount} 個檔案</span>}`（fileCount=0 = pre-S098a3-2 fallback signal 隱藏避免「0 個檔案」誤導）+ test fixture v 加 `fileCount: 3` default + AC-S117-1（顯示）/ AC-S117-2（fallback hide）。VersionList tests 7/7 PASS；full suite 195/195 PASS @ 41 files（0 regression）。第 13 次 single-tick XS/S ship；Round 36 backlog chain 2/3。 |
| Phase 5 | M113: Collection DTO field naming alignment `installs → installCount` (Bug AQ fix) | S118 | XS(2) | v3.10.4 | ✅ shipped 2026-05-04 cron Tick 11 (single-tick ship)。Atomic rename across 5 callsites：backend `CollectionSummary` record + CollectionControllerTest jsonPath；frontend `SkillCollection` type + CollectionsPage CollectionCard caller + CollectionsPage.test.tsx fixture × 3 entries。Frontend CollectionsPage tests 4/4 PASS @ 1.62s。**Round 36 backlog chain 完整收尾 3/3**（S119 + S117 + S118 從 2026-05-03 audit → 2026-05-04 全 ship）。第 14 次 single-tick XS/S ship；atomic rename pattern 範本（vs Jackson @JsonAlias 過渡），internal repo callers 一次到位避免 transitional tech debt。 |
| Phase 5 | M114: Skill list rating projection — SELECT include `average_rating, review_count` (Bug AR fix) | S119 | XS(2) | v3.10.2 | ✅ shipped 2026-05-04 cron Tick 9 (single-tick ship)。`SkillQueryService.search()` SQL SELECT 加 `average_rating, review_count`；`Skill.fromRow` 15-arg overload + 13-arg backward-compat delegate（**0 callsite migration**，對齊 S116 既驗）。E2E manual smoke：DB seed UPDATE → list 從 0/0 → 4.5/2，single GET 一致；SkillSearchTest 14 ACs（13 既驗 + S119-1 新加 raw SQL UPDATE seed pattern 配 @ReadOnlyProperty 字段）。第 12 次 single-tick XS/S ship；第 3 次 backward-compat overload pattern。LAB 員工瀏覽 skills list 看到 SkillCard rating 真實評分。 |
| Phase 5 | M115: E2E auth integration test — OAuth + visibility + ACL grant + download flow | S120 | M(8-10) | — | 📐 in-design — 2026-05-04 spec doc 寫成（user 2026-05-04 directive：「ACL 已實作 / 整合授權機制 / mock-oauth2-server 可用 / 資料清空後重新設測試 / A 上傳公開+私人 / 共享 B 唯讀 / 真實下載 / 看下載次數」）。Approach B：`@SpringBootTest(RANDOM_PORT)` + Testcontainers GenericContainer (mock-oauth2-server 3.0.1，對齊既有 OAuthMockE2ETest pattern) + `@DynamicPropertySource` 注入 issuer-uri + 既有 TestcontainersConfiguration (pgvector)；scenario-style 單一大 test method 跑 14 ACs flow（fetch JWT for A=dev-042 / B=viewer-007 → A 上傳 public+private → anonymous list 過濾 → A grant user:viewer-007:read → B list+download → assert download_count cross-user invariant）。對齊 S016 GIN ?| filter + S026 *:read + S076 download counter atomic + S116 visibility 既驗 infra。**重點**：暴露 read-side `findById`/`download` endpoint 缺 @PreAuthorize 之 known gap（per S114a plan）→ 走 `@Tag("DOCUMENT-S114A-GAP")` 紀錄 current behavior + 留 follow-up fix-spec backlog；不改任何 production code（純 test infra）。User-realistic + CI-friendly + 為後續 OAuth+ACL 相關 spec 提供回歸基線。Defer: revoke flow / multi-skill cross-pollution / concurrent download stress / Chrome MCP frontend full stack / JWT refresh / group&role principal pattern test（per §2.6 trim）。15 ACs（含 AC-1 setup + AC-2~14 scenario steps + AC-15 revoke 可 defer）。為 S114a / S121 (gap fix-spec) 提供 ship 後回歸驗證 baseline。 |
| Phase 5 | M116: SkillQueryService.search() row-level ACL filter (Bug AS fix — **LAB-blocker**) | S121 | S(4-5) | v3.8.4 | ✅ shipped 2026-05-04 cron Tick 2 (single-tick ship)。`SkillQueryService.search()` 注入 `CurrentUserProvider` + `AclPrincipalExpander`，加 `AND acl_entries ??\| :aclPatterns` SQL clause（對齊 S016 既驗 `??` escape + `SqlParameterValue(Types.ARRAY)` ARRAY bind 模式）。S116 visibility toggle 在 list endpoint **真實生效** — anonymous list 從 total=2 → total=1（PRIVATE 不再 leak）；B grant 後可見；revoke 後不見。**不加 admin bypass 路徑**對齊 S016 既驗 read 路徑走 expand pattern；admin 若需看 PRIVATE 須走 grant。SkillSearchTest 13/13 PASS（既有 11 不 regression + 新加 AC-S121-1/-2）；E2E manual smoke 6 case all PASS。**LAB-blocker 已解** — 員工封測時 PRIVATE skill 對非 grantee 不再 visible。第 3 次採用 AclPrincipalExpander + `??|` 雙 hack；第 6 次 single-tick XS/S ship。 |
| Phase 5 | M117: Read-side single-skill ACL gate (Bug AT fix — read-side chain) | S122 | S(4-5) actual | v3.8.5 | ✅ shipped 2026-05-04 cron Tick 3 (single-tick ship；XS→S due evaluator scope creep)。3 個 read endpoints (`getById` + `bundleInfo` + `getVersions`) 加 `@PreAuthorize("hasPermission(#id, 'Skill', 'read')")` 守則。**同 spec 內補修 DelegatingPermissionEvaluator 對 anonymous-read 過嚴**：S016 `authenticated()` 短路對 read 違反 S026「`*:read` 預設公開」設計；補 anonymous + read → `[*:read]` pseudo-principal 評估，PUBLIC skill 對 anonymous 仍可訪問；其他 permission 維持 fail-secure 短路。15-case E2E manual smoke all PASS（anon PUBLIC=200 / anon PRIVATE=401 / B no-grant=403 / B granted=200 / A owner=200 對 3 endpoints + S121 list regression）。getByAuthorAndName alias path → S124 follow-up（不同 SpEL signature）；download endpoint → S123 follow-up（同 chain）。第 7 次 single-tick XS/S ship；DelegatingPermissionEvaluator 第 1 次補 anonymous-read 路徑。 |
| Phase 5 | M118: Download endpoint ACL gate (Bug AU fix — read-side chain closer) | S123 | XS(2) | v3.8.6 | ✅ shipped 2026-05-04 cron Tick 4 (single-tick ship)。`downloadLatest` + `downloadVersion` 兩 method 加 `@PreAuthorize("hasPermission(#id, 'Skill', 'read')")` 守則。修補 anonymous 直打 PRIVATE skill download 拿到 zip body 含實際 SKILL.md 的 critical leak。**完成 read-side ACL chain**：S121 (list) + S122 (single GET / versions / bundle-info) + S123 (download / downloadVersion) **6 個 read endpoint 統一 ACL 守則**。Reuse S122 evaluator anon-read 路徑不需另修。E2E manual smoke 12/12 case all PASS（含 5 downloadLatest + 3 downloadVersion + 4 regression）；download_count invariant 維持（S076 atomic SQL 不受 ACL gate 影響）。第 8 次 single-tick XS/S ship；第 4 次採用 controller-level `@PreAuthorize` 既驗 pattern。 |
| Phase 5 | M119: getByAuthorAndName ACL gate (S122/S123 follow-up) | S124 | XS(2) | v3.10.5 | ✅ shipped 2026-05-04 cron Tick 12 (single-tick ship)。`@PostAuthorize("hasPermission(returnObject.id, 'Skill', 'read')")` resolve-then-check pattern — 對 alias path `/skills/{author}/{name}` 補 ACL 不需自訂 strategy；reuse 既驗 SkillPermissionStrategy SQL 路徑。E2E manual smoke 8/8 case PASS（anon PUBLIC=200 / anon PRIVATE=401 / nonexistent=404 / A owner=200 / B no-grant=403 / B granted=200 + regression id-based + list）。**read-side ACL chain 完整收尾**（S121 + S122 + S123 + S124 共 7 個 read endpoint 一致 ACL 守則）。第 15 次 single-tick XS/S ship；第 1 次採用 @PostAuthorize resolve-then-check pattern（vs 新加 strategy overload — 對齊「真的有第三 use case 才抽」原則）。 |
| Phase 5 | M121: Skill id format validation pre-PreAuthorize (Bug AX fix — LAB UX polish) | S126 | XS(2-3) | v3.10.7 | ✅ shipped 2026-05-04 cron Tick 15 (single-tick ship + Approach retry)。`@PathVariable String id → UUID id` 7 處（5 SkillQueryController + 2 SkillSubscriptionController）+ GlobalExceptionHandler 加 `@ExceptionHandler(MethodArgumentTypeMismatchException)` 翻譯 400 VALIDATION_ERROR。**Critical lesson**：Spring Security `@PreAuthorize` interceptor **早於** `@Validated + @Pattern` method validation interceptor — Approach B (initial attempt) E2E 顯示仍 401，trace 至 interceptor order 後 revert 至 Approach A (`@PathVariable UUID` 內建 converter，arg resolution 階段就 throw)。E2E 9/9 case PASS（invalid `null`/`not-a-uuid` → 400；valid UUID nonexistent → 401 security-first 不變；regression all OK）。**Round 39 backlog 完整收尾 2/2**（S127 + S126）。第 17 次 single-tick XS/S ship；GlobalExceptionHandler 第 20 個 handler；spec doc §2.1 Approach B rejected mid-implementation 紀錄 audit trail 透明化。 |
| Phase 5 | M122: NoResourceFoundException ErrorResponse 一致性 (Bug AY fix — Spring-level) | S127 | XS(1) | v3.10.6 | ✅ shipped 2026-05-04 cron Tick 14 (single-tick ship)。GlobalExceptionHandler 加 `@ExceptionHandler(NoResourceFoundException.class)` 翻譯 Spring 6+ trailing slash / 不存在 endpoint 預設 404 為標準 NOT_FOUND ErrorResponse shape；對齊既驗 NoSuchElementException → NOT_FOUND 路徑。E2E manual smoke 6/6 PASS（含 3 個 NoResourceFoundException 路徑 + 1 既驗 regression + 1 Tomcat-level known limitation 不 ship + 1 success regression）。**Tomcat-level `/skills//foo` 400 不 fix**（known limitation per spec §2.2 — Tomcat 直接拒絕 empty segment 不進 Spring flow；frontend 不會生 double-slash URL；安全 feature 不該繞過）。第 16 次 single-tick XS/S ship；GlobalExceptionHandler 第 19 個 handler 加；Tomcat-level vs Spring-level layer 區分 lesson 確立。 |
| Phase 5 | M123: CORS configuration (Bug AZ fix — LAB cross-origin deploy unblock) | S128 | XS(2-3) | v3.10.8 | ✅ shipped 2026-05-04 cron Tick 16 (single-tick ship)。**LAB-blocker** 已知 tech debt 自 tick 63 R20.4 補上。SkillshubProperties.Security 加 `Cors` record (allowedOrigins List + allowCredentials)；SecurityConfig 加 CorsConfigurationSource bean + http.cors() 啟用；path pattern `/api/**`；env var `SKILLSHUB_SECURITY_CORS_ALLOWED_ORIGINS` 顯式覆蓋 production allowlist。E2E manual smoke 6/6 PASS（OPTIONS allowed=200+5 CORS headers / GET allowed=Access-Control-Allow-Origin echo / OPTIONS disallowed=403 / download CORS / no-Origin no-echo / regression PUBLIC=200）。第 18 次 single-tick XS/S ship；CORS canonical pattern 確立；SkillshubProperties.Security 第 3 個 sub-record (OAuth + Lab + Cors)；LAB 部署 frontend 跨 origin unblock。 |
| Phase 5 | M124: Server compression (Bug BA polish) | S129 | XS(1) | — | 📋 planned — Mode B Round 40 (Tick 16) finding LOW。Spring Boot default `server.compression.enabled=false`；large skill list response (>10KB) 浪費 bandwidth。Fix：application.yaml 加 `server.compression.enabled=true` + `mime-types: application/json,text/plain` + `min-response-size=1024`。Single-tick XS=1。**非 LAB-blocker**；defer 至 production 部署觀察 bandwidth profile 後再啟用。 |
| Phase 5 | M125: Personal endpoints auth gate (Bug BB fix — LAB session integrity) | S130 | XS(1) | v3.10.9 | ✅ shipped 2026-05-04 cron Tick 17 (single-tick ship；Mode B finding 同 tick ship 第 2 次採用)。SecurityConfig OAuth branch `requestMatchers` 擴充 `/api/v1/me/**` + `/api/v1/notifications, /api/v1/notifications/**` require authenticated。修補 anonymous 透過 CurrentUserProvider lab-user fallback 共享 personal state（subscriptions / notifications / mark-read / preferences）的 LAB session integrity 漏洞。E2E manual smoke 14/14 PASS（6 anon-personal=401 + 4 auth=200 + 3 public regression=200 + 1 out-of-scope observation）。`/api/v1/me/**` 顯式 `/**` 子路徑必要 — `/me` ant pattern 不含 `/me/...`。第 19 次 single-tick XS/S ship；out-of-scope observation：anon POST /skills/{id}/subscribe 仍 201 (lab-user write)，架構 smell 但 LAB 影響小，留 future polish。 |
| Phase 5 | M126: Error code naming convention alignment (Bug BD fix — LAB UX polish) | S131 | XS(2-3) | — | 📋 planned — Mode B Round 42 (Tick 18) finding LOW UX/i18n。Backend GlobalExceptionHandler 16 UPPER + 10 lower_snake_case 混用；frontend `api-error-messages.ts` ERROR_MESSAGE_BUILDER 13 entries 全 UPPER → 10 lower codes 無 i18n 翻譯，frontend `localizeApiError` fallback 直接顯 raw "collection_not_found: <uuid>" 給 LAB 員工。違反 CLAUDE.md「UI 語言: 繁體中文」原則。Fix：backend rename 10 lower codes (FLAG/REQUEST/NOT_REQUEST/COLLECTION/SKILL/NOTIFICATION/NOT_NOTIFICATION/BUNDLE/INVALID_STATUS/INVALID_TOKEN) → UPPER；frontend mapping 加 10 個對應繁中翻譯。Atomic rename + frontend mapping 同 PR ship 對齊 Round 36 既驗 pattern。LOW LAB UX polish；non-LAB-blocker。 |
| Phase 5 | M127: CI — Cloud Build pipeline (build + push image) | S132 | XS(7) | — | ⏳ Dev — T01 PASS（cloudbuild.yaml + Gradle 解耦 + 03-build-push.sh 自帶 frontend build + docs sync）；T02 pending（manual GCP setup + first push smoke 待使用者執行）。接 S013 archive §7.7 tech debt（CI/CD 觸發 03/04 腳本）。Scope = build only（不含 deploy）：push to `main` → Cloud Build trigger → `node:22` step build frontend → `alpine` step copy `frontend/dist/` → backend static → `eclipse-temurin:25-jdk` step `./gradlew bootBuildImage` → declarative `images:` push 到 Artifact Registry（`<region>-docker.pkg.dev/<proj>/skillshub/skillshub:$SHORT_SHA`）。同步：從 `backend/build.gradle.kts` 移除 `npmInstall` / `npmBuild` / `copyFrontend` 三 task（解開「`bootRun` 跑 npm 失敗」痛點，本機 dev 改 `cd frontend && npm run dev`）；`scripts/gcp/03-build-push.sh` 加 `npm ci + npm run build + cp` 前置以保留本機 deploy 能力。Cloud Build runtime SA `skillshub-cloudbuild` 最小權限（`artifactregistry.writer` + `logging.logWriter`）；Developer Connect 2nd gen GitHub trigger。Out of scope（留 follow-up）：自動 deploy / tag → prod approval gate / PR check / build cache / SBOM publish / test gate。Spec：`docs/grimo/specs/2026-05-04-S132-ci-cloud-build.md`。 |
| Phase 5 | M120a: SkillSubscription backend infra (S125 split 1/3) | S125a | XS(4) | v3.9.0 | ✅ shipped 2026-05-04 cron Tick 6 (single-tick ship)。SkillSubscription aggregate (ADR-002 canonical：`@Version` + `AbstractAggregateRoot` + factory) + V14 migration `skill_subscriptions(id, skill_id, subscriber_id, created_at, version)` + UNIQUE(skill_id, subscriber_id) + 2 indexes + Repository (4 derived query) + Service (5 method：subscribe / unsubscribe / isSubscribed / findSubscribersOf / findSubscriptionsOfCurrentUser；idempotent；skill 存在檢查)。SkillSubscriptionServiceTest 7 ACs。第 3 次 community module aggregate；第 9 次 single-tick XS/S ship；spec split pattern 第 1 次採用 M→3XS。Dev gotcha 紀錄：新加 .sql 須 processResources 才 apply。 |
| Phase 5 | M120b: SkillSubscription endpoints + version-published listener (S125 split 2/3) | S125b | XS(3) | v3.10.0 | ✅ shipped 2026-05-04 cron Tick 7 (single-tick ship + Bug AW fix 同 spec)。`SkillSubscriptionController` 3 endpoints (POST/DELETE subscribe + GET /me/subscriptions) + `NotificationProjectionListener.onVersionPublished` 第 5 個 listener（self-action skip + preferences gate + UNIQUE idempotency）。**Bug AW critical fix**：DelegatingPermissionEvaluator.expandPrincipals 對 authenticated read 補 `*:read` pseudo-principal — S122 ship 漏此 case 致 LAB 員工拿 JWT 看不到 PUBLIC skills；本 tick S125b @PreAuthorize 在 subscribe 才 surface，同 spec 內補修對齊 anonymous-read 路徑（S122 既驗）。E2E manual smoke 10/10 case PASS — **PRD §285-§291 P9 SBE scenario 1 端到端首次完整 demo**（B subscribe → A PUT v2.0.0 → B 收通知）。第 10 次 single-tick XS/S ship；evaluator 第 2 次補 *:read（anon + auth 對稱）。 |
| Phase 5 | M120c: SkillSubscription frontend (S125 split 3/3) | S125c | XS(3) | v3.10.1 | ✅ shipped 2026-05-04 cron Tick 8 (single-tick ship)。frontend `api/subscriptions.ts` 3 helper + `useSubscription.ts` 4 hook (`useMySubscriptions` + `useIsSubscribed` derived + 2 mutation) + SkillDetailPage SkillHero 加 `SubscribeButton` 元件（Bell/BellOff toggle + 作者自己看不到 + pending state + aria-pressed）。`useIsSubscribed` derived from list contains 不另開 GET status endpoint（多 skill 共 cache）。Frontend tests 193/193 PASS @ 41 files；TypeScript tsc 0 errors。**S125 chain 完整 ship**（S125a + b + c 三 tick）— PRD §285-§291 P9 SBE scenario 1 端到端 demo-able；LAB 封測員工可從 UI self-service 訂閱 / 取消訂閱 flow。第 11 次 single-tick XS/S ship。 |
| Phase 5 | M111: Skill Visibility Toggle — public / private at create | S116 | S(8) | v3.8.3 | ✅ shipped 2026-05-03 cron Tick 35-36 (1 tick spec planning + 1 tick single-tick ship)。Approach B (derived from acl_entries 是否含 *:read；零 schema 變動)。Visibility enum (PUBLIC/PRIVATE 兩值 + defaultValue) + CreateSkillCommand 加 field + 4-arg backward-compat ctor delegate to 5-arg PUBLIC default + Skill.create factory 條件式 seed *:read（PUBLIC 加；PRIVATE 不加；author=null+PRIVATE → IllegalArgumentException）+ SkillCommandService 5-arg overload + SkillCommandController @RequestParam visibility=PUBLIC default。Frontend api/skills.ts Visibility type + uploadSkill 5-arg + PublishPage radio group fieldset 兩 option (公開 / 私人) + state。SkillAggregateTest 29/29 PASS（既有 24 + S116 新加 5 個 AC tests）+ SkillCommandServiceTest 2/2 + ModularityTests 2/2 + 全 frontend 193/193 PASS（0 regression；既有 4-arg uploadSkill caller 行為與 v3.x 完全一致）。第 2 次採用 backward-compat ctor delegate pattern（vs S098a3-2 cross-test migration 6 callsites — 0 callsite migration 即可 ship）；第 2 次 derived from existing column 模式（對齊 S038）。User mid-tick directive 2/2 完成。Defer (polish): visibility 變更後操作 / SkillDetail badge / MySkillsPage filter chip / SkillCreatedEvent visibility field / glossary 中英對照 / AC-3/4 端到端 @SpringBootTest。 |
