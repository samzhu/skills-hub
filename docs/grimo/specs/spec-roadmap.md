...
| Phase 4 | M92a3-2: Backend bundle-info endpoint | S098a3-2 | XS(2) split from S098a3 | — | 📐 in-design — 2026-05-03 spec doc 寫成。新增 `GET /api/v1/skills/{id}/bundle-info` 返回 `{filename, fileSize, fileCount, uploadedAt}`；V13 migration `skill_versions.file_count INTEGER NOT NULL DEFAULT 0`（既有 row 0 = legacy unknown，frontend hide 該欄；MVP 不 backfill）；上傳 pipeline zip extract phase 計算 entries 寫入；frontend `useBundleInfo` hook + PublishValidatePage strip 顯實值取代派生 placeholder（既有 placeholder 作 endpoint 失敗 fallback）。7 ACs。對齊 useSkill / useRequest enabled-gate canonical。 |
