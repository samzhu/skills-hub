...
| Phase 4 | M92e2: Reviews aggregate + ratings + SkillDetail Reviews tab | S098e2 | S(11) re-est from M(8) | — | 📐 in-design — 2026-05-03 spec doc 寫成；新建 `review` 模組（ADR-002 canonical pattern：Spring Data JDBC 充血聚合 + Modulith Outbox + AFTER_COMMIT projection）；3 endpoints (POST/GET/DELETE)；2 schema migrations（reviews 表 + skills 加 average_rating/review_count）；12 ACs (8 backend + 4 frontend：含 1-5 星 / 1 則 per user / hero+list UI / 提交 form happy path)；6 個產品決策定案（1-5 星、1-per-user、強制登入、無 helpful、Approach A 投影、無 moderation）。Listener 放置位置 (review-side vs skill-side) 由 implementer 視 Modulith verifier 決定（S112-T01 deviation 啟示）。Hypothesis 不阻塞 spec，無需 POC。為 S101b Impact Score 提供 rating sub-metric。 |
| Phase 4 | M92e3: Flag 回報流程 | S098e3 | M(7) split from S098e | — | 📋 planned — backend Flag aggregate + reviewer queue 整合 + frontend 回報 form |
...
| Phase 4 | M90f2: Collections full feature | S096f2 | M(10-12) split from S096f | — | 📋 planned — Collection aggregate + Modulith community module registration + 3 endpoints (POST create/install/GET single) + 2 domain events (CollectionCreated/CollectionInstalled) |
...
| Phase 4 | M90g2: Request Board full feature | S096g2 | M(10-12) split from S096g | — | 📋 planned — Request aggregate + Modulith community module + 3 endpoints (POST/vote/claim) + 3 domain events (RequestPosted/RequestVoted/RequestFulfilled) |
...
| Phase 5 | M106: Flag wiring full-stack — SkillDetail Flags tab + MySkills openCount metric | S112 | S(7) | — | 🚧 in-progress 3/4 — T01 backend (Tick 3) + T02 fe-infra (Tick 4) + T03 SkillDetail Flags tab (Tick 5：FlagsList extract 到獨立 component；FlagsList.test.tsx AC-1/AC-2 PASS @ 880ms) shipped：新建 `security/MeFlagsController` + `FlagService.countOpenFlagsForAuthor` + 4 tests (AC-5/AC-6/AC-7×2) 全 PASS；deviation：spec §4.1 寫「擴 MeController」但 Modulith 邊界 `shared → security` 反向不允許 → 改建獨立 controller 在 `security` 模組（與 skill/search 同 pattern）。T02-T04 frontend 串接待後續 ticks。原始 design：2026-05-03 page audit fresh round 發現 backend `FlagController` 已完整 ship 但前端兩處未串：(1) SkillDetailPage Flags tab 永遠 EmptyState；(2) MySkillsPage 「待處理回報」MetricCard 寫死 `value={0}`。本 spec 接 GET /skills/{id}/flags + 新加 GET /me/flags-summary（aggregate count）+ 移除 MySkillsPage 「平均評分」假 metric（等 S101a Quality Score）。7 ACs；4 tasks (T01 backend / T02 fe-infra / T03 SkillDetail / T04 MySkills)；execution order T01→T02→T03→T04。與 📋 S098e3（reviewer queue + POST 表單）/ 📋 S098e2（Reviews aggregate）不衝突。 |
