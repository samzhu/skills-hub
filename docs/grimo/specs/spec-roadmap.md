...
> 最後更新：2026-05-19（S197 QA REJECT-FIX；next `$planning-tasks S197`）
...
| S197 | 必填欄位即時提示 UX（Publish / Edit required mark + inline error，不等 submit 才顯示 native bubble） | S(8) | S176 ✅, S188 ✅, S195 ✅ | ⏳ Dev — QA REJECT-FIX 2026-05-19；AC-S197-3 `/publish` file mode 缺 inline file-required error；next `$planning-tasks S197` |
