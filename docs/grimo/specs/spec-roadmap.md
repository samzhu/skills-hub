...
| Phase 5 | M120c: SkillSubscription frontend (S125 split 3/3) | S125c | XS(3) | — | 📋 planned — S125b 後 frontend layer。SkillDetail page 加「訂閱」按鈕 + state（按鈕 toggle subscribe/unsubscribe + 顯示當前訂閱狀態）；新增 useSubscription hook 包 isSubscribed / subscribe / unsubscribe API helpers。Bell badge 自動 reflect 新通知（既有 30s poll path）；Notifications page 既有 filter chips 自動含 `versions` category（per S096h2 ship）。可 LAB 部署後補（frontend 整體 polish 候選）；非 LAB-blocking — 員工可從 API 測 flow，UI 是 nice-to-have。 |
