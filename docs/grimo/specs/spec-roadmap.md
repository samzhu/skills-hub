...
| Phase 4 | M92e3: Flag 回報流程 — POST form + reviewer queue + status mutations | S098e3 | S(8) re-est from M(7) | — | 🚧 in-progress 1/4 — T01 backend write flow shipped Tick 13（FlagStatus enum + canTransitionTo + FlagService updateStatus/listAllFlags/createFlag 加 reporter + FlagController PATCH/?status= + FlagAdminQueryController + 2 exceptions + GlobalExceptionHandler 加 mapping；FlagServiceTest 9/9 + FlagControllerTest 5/5 + ModularityTests PASS）。Pending：T02 frontend infra → T03 FlagsList CTA + FlagSubmitModal → T04 FlagsQueuePage + AppShell nav + /flags route。原始 design：補完 ✅ S112 (read 端) 對應的 write + reviewer 端。零 schema migration（status 既是 String 欄位，純 enum 擴充 OPEN→RESOLVED/DISMISSED）；13 ACs (8 backend + 5 frontend)；7 個 UX 決策（Feature First：任何登入用戶可看 reviewer queue；OPEN→RESOLVED/DISMISSED 雙 terminal；reportedBy 改 useMe.sub；submission Modal）。對齊 PATCH semantic for status update (Approach A)。為 S112 read 端補完 write loop。 |
...
| Phase 4 | M90g2: Request Board full feature — aggregate + voting + claim + fulfillment | S096g2 | S(11) re-est from M(10-12) | — | 📐 in-design — 2026-05-03 spec doc 寫成；ADR-002 canonical aggregate (Spring Data JDBC 充血 + Modulith Outbox)；7 endpoints (POST create / GET list?status=&sort= / GET single / POST vote toggle / POST claim / DELETE claim release / POST fulfill / DELETE own OPEN)；2 schema migrations (requests + request_votes UNIQUE join)；5 domain events (RequestPosted / RequestVoted / RequestClaimed / RequestReleased / RequestFulfilled)；17 ACs (14 backend + 3 frontend)；7 個 UX 決策（vote toggle 1 票 / 1 claimer exclusive / OPEN→IN_PROGRESS→FULFILLED + IN_PROGRESS→OPEN release / fulfill 必綁 PUBLISHED skillId / 只能 delete 自己 OPEN / 任何 user 可 vote+claim 只 claimer 可 fulfill+release / 只 requester 可 delete）。Vote count 走 raw SQL atomic UPDATE pattern (對齊 S076 download_count)。為 S096h2 Notifications 提供 5 個 event hook。 |
