...
| Phase 5 | M112: Frontend SkillVersion type sync `fileCount` (Bug AP fix) | S117 | XS(1) | — | 📋 planned — Mode B Round 36 (2026-05-03) finding。Frontend `SkillVersion` interface (`frontend/src/types/skill.ts:113`) 缺 `fileCount` field；backend SkillVersion.getFileCount() getter 自動序列化 expose 但 frontend tsc OK 不 fail；VersionList 顯版本歷史時無「N 個檔案」資訊；S098a3-2 ship 漏同步此 type。Fix：1 行 type addition + optional VersionList row 加「N 個檔案」display + .test.tsx fixture 同步。Single-tick XS。 |
