...
| S185 | Skill list/detail projection consistency（list visibility/version fields 對齊 detail） | XS(8) | S175 ✅, S177 ✅, S184 ✅ | ⏳ QA PASS — `./scripts/verify-all.sh` exit=0；release/deploy pending；Round 69 production API found same skill list returns PRIVATE/versionCount=0 while detail returns PUBLIC/versionCount=1 |
