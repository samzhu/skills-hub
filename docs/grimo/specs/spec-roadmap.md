...
| Phase 4 | M92e3: Flag 回報流程 — POST form + reviewer queue + status mutations | S098e3 | S(8) re-est from M(7) | — | 🚧 in-progress 2/4 — T01 backend (Tick 13) + T02 frontend infra (Tick 14：api/flags.ts 加 createFlag/fetchFlagsByStatus/updateFlagStatus + Flag.status 加 DISMISSED + lib/flag-labels.ts 加 DISMISSED label/style + useFlagsQueue hook；typecheck 0 error + FlagsList regression 2/2 PASS) shipped。Pending：T03 FlagsList CTA + FlagSubmitModal → T04 FlagsQueuePage + AppShell nav + /flags route。原始 design：補完 ✅ S112 (read 端) 對應的 write + reviewer 端。零 schema migration（status 既是 String 欄位，純 enum 擴充 OPEN→RESOLVED/DISMISSED）；13 ACs (8 backend + 5 frontend)；7 個 UX 決策（Feature First：任何登入用戶可看 reviewer queue；OPEN→RESOLVED/DISMISSED 雙 terminal；reportedBy 改 useMe.sub；submission Modal）。對齊 PATCH semantic for status update (Approach A)。為 S112 read 端補完 write loop。 |
