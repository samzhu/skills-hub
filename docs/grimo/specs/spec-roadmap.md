...
| Phase 4 | M90g2: Request Board full feature — aggregate + voting + claim + fulfillment | S096g2 | S(11) re-est from M(10-12) | — | 🚧 in-progress 3/4 — T01 backend aggregate (Tick 18) + T02 vote toggle (Tick 19) + T03 frontend infra (Tick 20：SkillRequest type 對齊 backend RequestResponse；7 helpers (createRequest / fetchRequest / toggleVote / claimRequest / releaseClaim / fulfillRequest / deleteRequest) + 5 types (VoteResult / ClaimResult / FulfillResult / CreateRequestBody / RequestsQuery)；useRequests + useRequest hooks；RequestBoardPage `req.votes` → `req.voteCount` 同步；npx tsc --noEmit PASS + useSkill.test.tsx 5/5 PASS @ 1.46s) shipped。Pending：T04 RequestBoardPage + components (CTA / VoteButton / RequestActionBar / CreateRequestModal)。原始 design：ADR-002 canonical aggregate (Spring Data JDBC 充血 + Modulith Outbox)；7 endpoints；2 schema migrations (requests + request_votes UNIQUE join)；5 domain events；17 ACs (14 backend + 3 frontend)；7 個 UX 決策。Vote count 走 raw SQL atomic UPDATE pattern (對齊 S076 download_count)。為 S096h2 Notifications 提供 5 個 event hook。 |
