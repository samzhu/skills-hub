...
| Phase 5 | M110: JWT + ACL Safety Design — schema evolution + graceful degradation | S115 | M(8-10) | — | 📋 planned — user 2026-05-03 mid-tick 直接需求：把 JWT Token 跟 ACL 控制設計落成「安全設計文件」（spec + ADR-006 雙寫）；core invariant：當 token claim 內容跟原先設計不一樣時（缺欄位 / 型別變動 / 新欄位）系統不直接壞掉，走 graceful degradation（缺 claim → fall back 安全保守值，例：roles=[] / groups=[]）。Scope 含 (1) 既有 CurrentUserProvider 三 branch 路徑 audit + 缺值 fallback 規格化；(2) JWT claim schema 文件化（roles / groups / sub / company_id / dept_id 等）+ optional vs required 標註；(3) 既有 ACL 規則（principal types：user / role / group / company / `*:read` synthetic public）安全行為 matrix；(4) Spring Security OAuth2 ResourceServer JwtDecoder error → 401 vs 403 vs degradation；(5) 端到端 negative tests（壞 token / 缺 sub / 缺 roles）。Depends S114a 為 ACL projection 落地後參照基礎；可 parallel ship。 |
