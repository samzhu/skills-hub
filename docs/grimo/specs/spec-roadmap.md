...
| Phase 4 | M92e2: Reviews aggregate + ratings + SkillDetail Reviews tab | S098e2 | S(11) re-est from M(8) | — | 🚧 in-progress 3/4 — T01 backend (Tick 8) + T02 projection (Tick 9) + T03 frontend infra (Tick 10：api/reviews.ts + useReviews + RatingStars readonly/interactive + Skill type 加 averageRating/reviewCount；RatingStars.test.tsx 5/5 PASS) shipped；T04 SkillDetailPage Reviews tab pending。原始 design：2026-05-03 spec doc 寫成；新建 `review` 模組（ADR-002 canonical pattern：Spring Data JDBC 充血聚合 + Modulith Outbox + AFTER_COMMIT projection）；3 endpoints (POST/GET/DELETE)；2 schema migrations（reviews 表 + skills 加 average_rating/review_count）；12 ACs (8 backend + 4 frontend：含 1-5 星 / 1 則 per user / hero+list UI / 提交 form happy path)；6 個產品決策定案（1-5 星、1-per-user、強制登入、無 helpful、Approach A 投影、無 moderation）。Listener 放置位置 (review-side vs skill-side) 由 implementer 視 Modulith verifier 決定（S112-T01 deviation 啟示）。Hypothesis 不阻塞 spec，無需 POC。為 S101b Impact Score 提供 rating sub-metric。 |
