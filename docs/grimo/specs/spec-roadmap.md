...
| Phase 5 | M120: SkillSubscription + version-published notification (Bug AV fix — **LAB 封測 user-visible gap**) | S125 | M(10-12) split candidate | — | 📋 planned — Mode B Round 38 (2026-05-04) finding HIGH。**PRD §285-§291 P9 SBE scenario 1 dependency 完全未實作**：`SkillSubscription` aggregate / V14 schema (skill_subscriptions table) / `POST /skills/{id}/subscribe` endpoint / `NotificationProjectionListener.onVersionPublished` 全缺。Glossary 已定義 `SkillSubscription` 但 codebase 0 實作；S096h2 ship notification infra 走 author-recipient (4 listener) 而非 subscriber-recipient。LAB 封測時員工**無法 demo「訂閱 skill → 收新版通知」flow**。Fix split 候選：S125a backend infra (XS=4 — aggregate + V14 + repo + service) / S125b listener + endpoints (XS=3 — POST/DELETE subscribe + onVersionPublished listener subscribe SkillVersionPublishedEvent + UNIQUE idempotency) / S125c frontend (XS=3 — SkillDetail subscribe button + state)。對齊 NotificationProjectionListener 既驗 4 listener pattern + AbstractAggregateRoot 既驗 ADR-002 canonical。建議 LAB 部署前 ship 至少 S125a/b 讓員工可測 flow。 |
