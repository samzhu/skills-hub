...
> 最後更新：2026-05-16（S183/S184 shipped v4.65.0；S180 native logs clean, logged-in Chrome UI verification blocked；S185 in-design from Round 69 production API mismatch）
...
| S178 | Browse Search Entry Point Cleanup（`/browse` 是唯一搜尋入口；刪 `/search` + intent summary；搜尋框有字只打 semantic） | S(9) | S177（ordering-only release safety） | ⏳ in-progress — T01/T02 PASS: `/browse` search now sends semantic-only debounced requests；T03/T04 pending for `/search` route + intent-summary removal |
