...
| Phase 5 | M99: EmptyState invite-tone steps decoupling | S105 | XS(3) | — | 📋 planned — Mode B Round 10 (Chrome MCP focus+Space tab nav) audit: EmptyState `invite` tone hardcode `['打包','自動掃描','發佈','追蹤']` 4-step strip，5 callsite 中只有 MySkillsPage 真符合 publish onboarding context；其他 4 處 (Collections/Requests/Reviews tab/Search no-query) 顯無關 steps。Frontend-only fix: 抽 optional `steps?: string[]` prop + MySkillsPage opt-in。寫於 2026-05-03 cron Tick 8 (Spec-Only-Handoff). |
