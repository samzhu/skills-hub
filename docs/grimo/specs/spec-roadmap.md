...
> 最後更新：2026-05-20（S204/S205 planned）
...
| S204 | OAuth Login Error Page（OAuth callback 失敗導到 `/auth/error`，不再顯示 Spring default `/login?error`） | S(5) | S139 ✅, S152 ✅ | 📋 planned — T01 backend failure handler / T02 frontend page / T03 route + docs sync |
...
| v4.88.0 | S204 (OAuth Login Error Page) | 📋 planned — `/login?error` 改導 Skills Hub `/auth/error`；後端 safe failure handler + 前端繁中可操作錯誤頁 |
