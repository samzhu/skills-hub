...
| Phase 5 | M118: SkillQueryController.download @PreAuthorize (Bug AU fix) | S123 | XS(2) | — | 📋 planned — Mode B Round 37 finding HIGH。Download endpoint 缺 ACL 守則；anonymous 直打 `/api/v1/skills/{private-id}/download` 拿到 zip body 含實際 SKILL.md 內容（敏感內容洩漏）。Fix：加 `@PreAuthorize("hasPermission(#id, 'Skill', 'read')")`；download_count 累計 invariant 不變（仍 atomic SQL）。Single-tick XS。對齊 S122 pattern，補完 S121/S122/S123 三 endpoint ACL 一致性。 |
