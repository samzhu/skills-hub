...
| Phase 5 | M114: Skill list rating projection — SELECT include `average_rating, review_count` (Bug AR fix) | S119 | XS(2) | — | 📋 planned — Mode B Round 36 (2026-05-03) finding (MEDIUM)。`SkillQueryService.search()` raw JDBC SELECT clause 缺 `average_rating, review_count` → list endpoint 永遠回 averageRating=0 / reviewCount=0；single endpoint findById 走 Spring Data JDBC auto-load 回真值；frontend SkillCard 顯 rating 星星永遠 0（user-visible bug）。S098e2 ship review averageRating projection 後此 list endpoint 漏 update SELECT。Fix：SQL 加 2 column + RowMapper 從 ResultSet read 兩欄 + 對應 Skill ctor 5 + 6 ordinal index update + frontend SkillCard rating display 驗證。Single-tick XS。 |
