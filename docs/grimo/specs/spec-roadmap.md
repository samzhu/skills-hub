...
> 最後更新：2026-05-12（S161c ship — V19 backfill SQL strip reviews/flags/collections 4 欄位既存 HTML payload；Testcontainers 啟動驗證 syntax + run-through）
...
| S148f | cyclonedx-bom 3.2.4 + nativeCompile 衝突修復 — 升 plugin 4.x / 換 SPDX / 隔離 task graph（POC 決定）；ship native production deploy 才會擋到 | XS(3) — POC 結果可漲 S | 上游修復 ＋ SBOM 消費者 ＋ native deploy 任一觸發 | ⏸ deferred 2026-05-10 — POC H1 reject（cyclonedx 無 4.x；上游 issue #821 open 無修復）；無 SBOM 消費者 + 無 native deploy 計畫 → 不划算 |
