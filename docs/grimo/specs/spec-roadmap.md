...
| Phase 5 | M102: Vite dev proxy for SpringDoc + footer API link UX | S108 | XS(2) | — | 📋 planned — Mode B Round 13 (curl 對比 dev :5173 vs backend :8080) audit: vite.config.ts proxy 只有 `/api/v1/*`，footer 「API」link `/v3/api-docs` 在 dev 環境 fallback SPA NotFoundPage（prod single-port 不受影響）。Frontend-only fix: vite proxy 加 SpringDoc 兩條 + footer link 從 raw JSON 改 swagger-ui (UX polish)。寫於 2026-05-03 cron Tick 14 (Spec-Only-Handoff). |
