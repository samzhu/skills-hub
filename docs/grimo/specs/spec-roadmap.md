...
| Phase 4 | M90f2: Collections full feature | S096f2 | M(13) re-est from M(10-12) | — | 🚧 task-planned (Tick 拆 4 BDD task files；T01 backend Collection aggregate + V12 schema + community @ApplicationModule + 2 events + SkillRepo extension → T02 CollectionService + Command/Query controllers + 2 exceptions + GlobalExceptionHandler + RequestService caller migration → T03 frontend api/skills.ts 3 helper + 2 hooks (useCollections + useCollection) → T04 CreateCollectionModal + InstallButton (loop browser download) + CollectionsPage CTA enable + AC-10/11/12 tests)。原始 design：ADR-002 canonical aggregate (Spring Data JDBC 充血 + @MappedCollection skills + Modulith Outbox + AbstractAggregateRoot for registerEvent + @Version for INSERT/UPDATE 區分 — S096g2 既驗，spec §4.3 Persistable 範本被 deviate)；4 endpoints；2 schema migrations；2 domain events；12 ACs (9 backend + 3 frontend)。Install 走 **Approach C** (frontend orchestration: 後端 record event + bump install_count + 回 N 個 download URLs；frontend loop trigger N 個 browser download；reuse 既有 GET /skills/{id}/download 自然累計 download_count)。community module 借此 spec 補 `@ApplicationModule` 正式 register (allowedDependencies = shared::events/api/security + skill::domain)。Risk filter (PRD §P7 SBE Scenario 3) defer 至 S096f3 polish。Skill picker UI 漂亮版 (MVP 走 textarea UUID list) + Collection detail page + edit/delete defer。為 S101b Impact Score 提供 install event hook。 |
