...
```
S019 ─▶ S020 ─▶ S021         Phase 2.5（Project Infra · M17 · 23 pts）
                  │
                  ▼
...
|------|------|-------|--------|------|--------|
| 1 | S019 | JaCoCo coverage gate + 80% line threshold | XS(5) | — | ⏳ Design |
| 2 | S020 | Verification command registry + `scripts/verify-all.sh` | S(10) | S019 | ⏳ Design |
| 3 | S021 | Phase 2 doc-sync — PRD.md + architecture.md | S(8) | — (可與 1/2 平行) | ⏳ Design |
| 4 | S016 | Row-Level ACL 基礎建設（JSONB acl_entries + GIN）| M(13) | S014 ✅ | 🔲 Backlog |
| 5 | S017 | ACL-Aware 語意搜尋（PgVectorStore + ACL SQL composition）| S-M(11) | S016 | 🔲 Backlog |
| 6 | S018 | Skill Aggregate 充血演化 + SKILL.md 對齊 + Suspend/Reactivate Events | M(13) | S014 ✅ + S016（graceful degrade `hasRole('admin')` 占位）| ⏳ Design — `specs/2026-04-27-S018-skill-aggregate-rich-domain.md`（revised 2026-04-28）|
...
| **Phase 2.5** | **M17: Project Infra** | **S019, S020, S021** | **XS(5)+S(10)+S(8) = 23** | **190** | **🔲 Planning** |
| **Phase 2** | **M14: Row-Level ACL 基礎建設** | **S016** | **M(13)** | **203** | **🔲 Backlog** |
| **Phase 2** | **M15: ACL-Aware 語意搜尋** | **S017** | **S-M(11)** | **214** | **🔲 Backlog** |
| **Phase 2** | **M16: Skill Aggregate 充血演化 + SKILL.md 對齊** | **S018** | **M(13)** | **227** | **⏳ Design** |

**MVP（v1.0.0）**：14 specs / 147 story points 已完成 🎉
**Phase 1（PostgreSQL 遷移 v1.1.0）**：1 spec / 20 story points 已完成（S015 absorbed）
**Phase 2.5（Project Infra）**：3 specs / 23 story points 規劃中 — 補 S014 ship 後浮出的 pre-existing project gaps + Phase 2 doc-sync
...
S019 (JaCoCo gate) ──▶ S020 (verify registry + verify-all.sh)
...
|---|------|--------|--------------|--------|
| S019 | JaCoCo coverage gate plugin + 80% line threshold | XS(5) | — | ⏳ Design — `specs/2026-04-27-S019-jacoco-coverage-gate.md` |
| S020 | Verification command registry + `scripts/verify-all.sh`（含 `bootRun -x processAot` workaround 編入 registry）| S(10) | S019（jacocoTestCoverageVerification 列入 registry）| ⏳ Design — `specs/2026-04-27-S020-verify-registry-and-script.md` |
