...
| Phase 5 | M113: Collection DTO field naming alignment `installs → installCount` (Bug AQ fix) | S118 | XS(2) | — | 📋 planned — Mode B Round 36 (2026-05-03) finding。`CollectionSummary.installs` (list endpoint) vs `CollectionDetail.installCount` (single endpoint) — 同 entity 同 metric 兩 field name；frontend `SkillCollection.installs` vs `CollectionDetail.installCount` 跟著走。Fix：rename CollectionSummary.installs → installCount；frontend SkillCollection type 同步；caller (CollectionsPage CollectionCard `{collection.installs.toLocaleString()}`) migration。Affects 1 backend record + 1 frontend type + 1 caller。Single-tick XS。 |
