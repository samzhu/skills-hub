...
| Phase 5 | M121: Skill id format validation pre-PreAuthorize (Bug AX fix — LAB UX polish) | S126 | XS(2-3) | — | 📋 planned — Mode B Round 39 (2026-05-04 Tick 13) finding LOW (UX confusion)。Invalid / nonexistent skill id（如 `null`、`undefined`、不存在的 UUID、非 UUID 格式）走 Spring Security 預設 fail-secure 路徑回 401（anon）/ 403（auth）— 對 LAB 員工 typo URL 反饋會混淆，應為 404 / 400 invalid id format。Affected: getById / bundleInfo / getVersions / downloadLatest / downloadVersion / subscribe 8 個 @PreAuthorize-protected endpoints。Fix：path variable UUID format validation（@Pattern 或 controller-method-pre-check）→ invalid format 400；nonexistent valid UUID 仍走 method body NoSuchElementException → 404。Trade-off 落點：UX（前置 validation）vs 嚴格 security-first。**LAB UX polish；非 LAB-blocker**。 |
