...
| 問題回報 | Community Flag | `Flag` | 使用者對已上架技能提交的問題回報；程式碼與 API 保留 `Flag` 命名，使用者介面顯示為「問題回報」 |
...
| 端對端測試 | E2E Test | `*.spec.ts` (Playwright) | 透過 browser 模擬使用者完整 user journey 的測試；S202 後 V07 目標是 production packaged app image（per ADR-007 + ADR-008） |
| 測試夾具狀態 | Fixture Profile | `@profile-<name>` tag | E2E 測試的初始資料狀態類型：empty / single / paged / full / mixed-visibility / multi-role / boundary；由 `e2e/results/fixtures.json` manifest 指向實際 skill/request/user fixture |
| 測試資料種子 | Test Data Seed | `FixtureRunner` / `FixtureManifest` | `e2e/fixtures` 外部 runner 準備資料：aggregate data 走正式 `/api/v1/*`，projection-only data 才能經 DB guard 寫入 disposable DB；production app 不提供測試 seed endpoint |
