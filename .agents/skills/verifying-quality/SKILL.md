...
description: >
  ...
  Use when all tasks pass, auto-verify raises issues, or QA review is
  requested. Don't use for implementation, release shipping, or project
  planning before a verifiable change exists.
...
2. **Deterministic verification script** — `scripts/verify-release.sh`,
   the canonical release QA gate for this project.
...
**MUST run `scripts/verify-release.sh`.** It is the canonical release QA
gate for this project. Do not choose another verification wrapper. The
script handles command ordering, environment detection, skip logic, and
...
- Write timestamped output to one latest-run log file: truncate it at
  script start, then append sections only during that run. Use rotated
  filenames only when historical logs are explicitly requested.
...
Record the command, log path, summary counts, verdict line, and only the
relevant failure excerpt. Do not paste full `verify-release.log` into
spec files or handoff notes; the local log file is the full evidence.
