...
2. **Round 1: Dispatch breadth agents in parallel.** Budget per sub-agent: 10 tool calls max. 3-point specs: 2–3 agents. 8+ point specs: 3–5 agents. Include at minimum: (a) org-level repo inventory if spec touches a GitHub org, (b) one agent per pinned library for API surface listing.
