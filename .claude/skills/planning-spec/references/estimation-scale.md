...
Move **up one Fibonacci step** at final story point confirmation when the implementation
...
- **Three scoring moments**:
  1. **Initial estimate** — choose `story_points` from roadmap scope before
     grill.
  2. **Planning adjustment after grill** — `/planning-spec` may shift `story_points`
     after questions reveal hidden complexity or simplify scope.
  3. **Final confirmation at ship** — `/shipping-release` confirms
...
- **Why final confirmation at ship matters:** The roadmap is the project's delivery
...
Python calculation on 2026-06-02 for
`docs/grimo/marketing-outcome.md`, using `ccusage 20.0.6` and
`tools/calculate_story_points.py`.

The period's Codex and Gemini activity belonged to `skills-hub`; Claude was
filtered to the `skills-hub` project. The token/cost formula is applied per
...
| Outcome points | 1,305 | Fibonacci story points through `v4.86.0 / S202`; generated by `tools/calculate_story_points.py` from story point records |
| Counted specs | 235 | `META`, cancelled, superseded, deferred, and rolled-up child specs count as 0 |
| Source coverage | 252 files / 4,482,923 bytes | Archive spec files are read via Python `read_bytes()`; duplicate archive files for the same SpecID are merged into that SpecID record |
| Weekly delivery | 351.3 points/week | `1305 / 26 * 7` |
| Cost per point | $2.58 | `$3362.38176865 / 1305` |

Roadmap cross-check:

| Roadmap denominator | Counted specs | Story points | Use when |
|---|---:|---:|---|
| Through `v4.86.0 / S202` | 235 | 1,305 | Marketing outcome date range: 2026-04-24 to 2026-05-19 |
| Current roadmap through `v4.90.0 / S206` | 239 | 1,347 | Includes work after the `ccusage` date range, so do not pair with the 2026-05-19 cost total |
...
story point accounting above are the current source of truth.
