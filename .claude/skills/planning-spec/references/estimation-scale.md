...
| Outcome points | 1,305 | Fibonacci story points through `v4.86.0 / S202`; generated by `tools/reestimate_story_points.py` after full archive spec reads |
| Counted specs | 235 | `META`, cancelled, superseded, deferred, and rolled-up child specs count as 0 |
| Archive read audit | 252 files / 4,482,923 bytes | Every archive spec file is read in full via Python `read_bytes()`; duplicate archive files for the same SpecID are merged into that SpecID audit record |
