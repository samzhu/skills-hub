...
- 這是什麼類型的錯誤？（bean creation / network / type / permission / config / behavior 不對）
- 涉及哪個 phase？（compile / test / build / deploy / runtime / startup / 線上）
- 之前看過類似的嗎？（檢查 `references/` 是否已有 case study）

#### 0.2 Search-Self-First：先懷疑自己的 config

**第 1 步永遠不是研究 framework，是 grep 自己**。簡單解釋優先於複雜解釋（Occam）：你或前人很可能在 own config 把 feature 關掉了。

```bash
grep -rn "<feature-keyword>" src/main/resources/ build.gradle.kts CLAUDE.md
```

這 2 秒的步驟 90% 場景能直接終結 debug。沒做 = 把學費付給 framework rabbit hole。

#### 0.3 Project memory consult：讀「為什麼這樣設」

你或前人很可能已經踩過同類問題、留下 lessons。**不讀 = 重付 learning tax**：

```
[ ] 同檔 / 同 module 的 comment / docstring / class Javadoc — 「為什麼這樣設」
[ ] CLAUDE.md / project README — architecture decisions
[ ] .claude/handovers/archive/ 最新 1-2 個 handover note — 上次 session known issues
...
### Phase 5.5：Pattern-Recognition-on-2nd（session 內 meta-skill）

**Session 觀察到「形狀相似的 bug」第 2 次出現** → 停下寫一句 rule，第 3+ 次直接套。
...
**為什麼必要**：人對 pattern 的識別需要「先寫下來」這個動作來固化，光靠記憶會漏。
寫下 → 自動套用；沒寫 → 第 3、4、5 次同 pattern 照樣繞遠路。

實戰例：S133 native runtime「AOT-time disable leak runtime」pattern 出現 4 次，前 3 次沒寫
rule，每次都從零 debug。詳 `references/case-study-spring-native-runtime.md` 原則 2。
...
| **懷疑外面前不 grep 自己** | 任何「X feature 不 work」直接研究 framework | 停。先 `grep keyword own/config/*` 排除「自己關掉」（Phase 0.2 Search-Self-First）|
| **不讀 project memory 直接 debug** | 同檔 comment / handover note / git log 沒看就動手 | 停。30 秒 consult project memory（Phase 0.3）|
| **Session 內同 pattern 第 3 次仍從零 debug** | 「形狀相似的 bug」第 2 次出現沒寫 rule | 停。寫一句 rule，第 3+ 次直接套（Phase 5.5 Pattern-Recognition-on-2nd）|
| **User 重啟同指令但你以為 done** | 同一指令 user 開 ≥ 2 次 | 停猜。問 user「your done = which scope: A/B/C」（Read-User-Intent）|
...
- `references/case-study-spring-aot.md` — Spring Boot 4 + Java 25 + Cloud Build AOT debugging 完整實戰（強調 Phase 1/2/6 執行細節）：6 次 CI 失敗 → 本機重現 → 因果鏈追蹤 → research agent 找到 issue #47781 → 5 次 attempt 累積 noise → 突破後 bisect 收斂到 minimal fix
- `references/case-study-spring-native-runtime.md` — Spring Native Runtime 16 個 build cycle 實戰（強調 Phase 0 / Phase 5.5 / 通用原則）：5 個通用原則完整萃取（Search-Self-First、Pattern-Recognition-on-2nd、Project-Memory-Consult、Read-User-Intent、Bisect-Judgement）+ 反模式疊加災難分析
