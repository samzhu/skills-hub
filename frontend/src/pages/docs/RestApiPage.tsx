...
export function RestApiPage() {
  return (
    <DocsLayout>
      ...
      <h1 className="text-[30px] font-semibold tracking-tight text-[#EEECEA]">REST 參考</h1>
      <p className="mt-3 text-[18px] leading-relaxed text-[#A8A49C]">
        Skills Hub 提供 OpenAPI 3.1 spec 與 Swagger UI（基於 SpringDoc 3.x；
        <code className="rounded bg-[#171719] px-1 py-0.5 font-mono text-[16px] text-[#EEECEA]">version: openapi_3_1</code> 設定）— local profile 下完整 schema 在
      ...
      <Callout>
        ...
        <code className="rounded bg-[#171719] px-1 py-0.5 font-mono text-[16px] text-[#EEECEA]">@PreAuthorize</code>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <tr key={i} className={i > 0 ? 'border-t border-[rgba(255,255,255,0.06)]' : ''}>
                ...
                <td className="px-3 py-2 align-top font-mono text-[16px] text-[#EEECEA]">{r.path}</td>
...
function Callout({ children }: { children: React.ReactNode }) {
  return (
    <div className="mt-4 rounded-md border p-3 text-[16px] leading-relaxed text-[#A8A49C]" style={{ backgroundColor: 'rgba(239,159,39,0.08)', borderColor: 'rgba(239,159,39,0.20)' }}>
      {children}
    </div>
  )
}
