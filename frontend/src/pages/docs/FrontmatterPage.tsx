...
export function FrontmatterPage() {
  return (
    <DocsLayout>
      ...
      <Callout>
        加了 <code className="rounded bg-[#171719] px-1 py-0.5 font-mono text-[12px] text-[#EEECEA]">allowed-tools</code> 或
        <code className="ml-1 rounded bg-[#171719] px-1 py-0.5 font-mono text-[12px] text-[#EEECEA]">scripts/</code> 資料夾就會觸發風險掃描；
        詳細層級規則見{' '}
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.name} className="border-t border-[rgba(255,255,255,0.06)] align-top">
              ...
              <td className="px-3 py-2 font-mono text-[12px] text-[#A8A49C]">{r.type}</td>
              <td className="px-3 py-2 text-[12px] text-[#A8A49C]">{r.limit}</td>
