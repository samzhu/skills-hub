  ...
  return (
    <DocsLayout>
      ...
      <p className="mt-3 text-[15px] leading-relaxed text-[#A8A49C]">
        ...
        <code className="mx-1 rounded bg-[#171719] px-1 py-0.5 font-mono text-[12px] text-[#EEECEA]">domain_events</code> 表（JSONB payload + aggregate_id + sequence
        嚴格遞增）。本頁列出 publisher 主要 event 結構供日後 webhook 訂閱使用。
      </p>

      <H2>共用包裝</H2>
      <pre className="mt-3 overflow-x-auto rounded-md border border-[rgba(255,255,255,0.06)] bg-[#0F0F12] p-4 font-mono text-[12px] leading-relaxed text-[#EEECEA]">
{`{
  "eventId": "uuid",
...
function Event({ type, body, payload }: { type: string; body: string; payload: string }) {
  return (
    <div className="mt-3 rounded-md border border-[rgba(255,255,255,0.06)] bg-[#0F0F12] p-3">
      ...
      <p className="mt-1.5 text-[13px] leading-relaxed text-[#A8A49C]">{body}</p>
