...
export function YourFirstSkillPage() {
  return (
    <DocsLayout>
      ...
      <h1 className="text-[30px] font-semibold tracking-tight text-[#EEECEA]">撰寫你的第一個技能</h1>

      <div className="mt-2 flex flex-wrap items-center gap-2 text-[16px] text-[#A8A49C]">
        <span>2 週前更新</span>
        <Dot />
        <span>閱讀 5 分鐘</span>
        <Dot />
...
└── assets/       [optional]
    └── examples.yaml`}</CodeBlock>
      ...
      <div className="mt-10 rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#0F0F12] p-5">
        <p className="text-[18px] font-semibold text-[#EEECEA]">準備發佈了嗎？</p>
        <p className="mt-1 text-[16px] text-[#A8A49C]">
      ...
      {/* §7 Prev/Next nav */}
      <nav className="mt-8 flex items-center justify-between border-t border-[rgba(255,255,255,0.06)] pt-5 text-[16px]">
...
function H2({ anchor, children }: { anchor: string; children: React.ReactNode }) {
  return (
    <h2 id={anchor} className="mt-10 text-[22px] font-semibold tracking-tight text-[#EEECEA]">
...
function CodeBlock({ filename, children }: { filename: string; children: string }) {
  return (
    <div className="mt-3 overflow-hidden rounded-md border border-[rgba(255,255,255,0.06)]">
      ...
      <pre className="overflow-x-auto bg-[#0F0F12] p-3 text-[16px] leading-relaxed">
...
function Callout({
  ...
  return (
    <div
      className="mt-4 flex items-start gap-2.5 rounded-md border p-3 text-[16px] leading-relaxed"
  ...
  return (
    <div className="mt-3 rounded-md border border-[rgba(255,255,255,0.06)] bg-[#0F0F12] p-4">
      <div className="flex flex-wrap items-center gap-2">
        <code className="rounded-sm bg-[#171719] px-2 py-0.5 font-mono text-[16px] text-[#EEECEA]">
          {name}
        </code>
        {tags.map((t, i) => (
      ...
      <p className="mt-2 text-[16px] leading-relaxed text-[#A8A49C]">{children}</p>
    </div>
  )
}
...
function RiskRow({
  ...
  return (
    <div className="flex items-start gap-3 rounded-md border border-[rgba(255,255,255,0.06)] bg-[#0F0F12] p-4">
      ...
      <p className="flex-1 text-[16px] leading-relaxed text-[#A8A49C]">{children}</p>
    </div>
  )
}
