...
export function UploadValidatePage() {
  return (
    <DocsLayout>
      ...
      <h1 className="text-[28px] font-semibold tracking-tight text-[#EEECEA]">上傳與驗證</h1>
      ...
      <H2>四步流程</H2>
      <Step n={1} title="上傳 zip 套件" body="作者在 /publish 拖放 bundle zip + 填寫版本號 / 分類。檔案大小 ≤ 5MB；解壓檔案數 ≤ 50。" />
      ...
      <ul className="mt-2 list-disc space-y-1 pl-5 text-[14px] text-[#A8A49C]">
        <li>frontmatter 缺 <code className="rounded bg-[#171719] px-1 py-0.5 font-mono text-[12px] text-[#EEECEA]">name</code> 或 <code className="rounded bg-[#171719] px-1 py-0.5 font-mono text-[12px] text-[#EEECEA]">description</code></li>
        <li>name 含大寫或特殊字元（規範要求 <code className="rounded bg-[#171719] px-1 py-0.5 font-mono text-[12px] text-[#EEECEA]">[a-z0-9-]</code>）</li>
...
function Step({ n, title, body }: { n: number; title: string; body: string }) {
  return (
    <div className="mt-3 flex gap-3 rounded-md border border-[rgba(255,255,255,0.06)] bg-[#0F0F12] p-3">
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-foreground text-[12px] font-medium text-background">{n}</div>
      <div>
        <p className="text-[14px] font-medium text-[#EEECEA]">{title}</p>
        <p className="mt-1 text-[13px] leading-relaxed text-[#A8A49C]">{body}</p>
      </div>
    </div>
  )
}
