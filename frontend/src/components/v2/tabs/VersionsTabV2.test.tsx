...
const versions: SkillVersion[] = [
  { id: 'v2', skillId: 's1', version: '2', fileSize: 4096, fileCount: 3, publishedAt: new Date(Date.now() - 86400000 * 2).toISOString() },
  { id: 'v1', skillId: 's1', version: '1', fileSize: 2048, fileCount: 2, publishedAt: new Date(Date.now() - 86400000 * 30).toISOString() },
]
...
describe('VersionsTabV2', () => {
  ...
  it('AC-S188-7: version labels shown in mono font without fixed format assumptions', () => {
    render(<VersionsTabV2 versions={versions} />)
    const nums = screen.getAllByTestId('version-num')
    expect(nums[0].textContent).toBe('v2')
    expect(nums[1].textContent).toBe('v1')
  })
